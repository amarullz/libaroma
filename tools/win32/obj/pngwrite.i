...
# 132 "../../../libs/png/pngwrite.c"
      if (!(info_ptr->colorspace.flags & 0x8000) &&
         (info_ptr->colorspace.flags & 0x0008) &&
         (info_ptr->valid & 0x0001))
         png_write_gAMA_fixed(png_ptr, info_ptr->colorspace.gamma);
# 144 "../../../libs/png/pngwrite.c"
      if (!(info_ptr->colorspace.flags & 0x8000) &&
         (info_ptr->valid & 0x1000))
      {

            if (info_ptr->valid & 0x0800)
...
png_write_info(png_structrp png_ptr, png_const_inforp info_ptr)
{
   ...
   if (info_ptr->valid & 0x0020)
      png_write_bKGD(png_ptr, &(info_ptr->background), info_ptr->color_type);



   if (info_ptr->valid & 0x0040)
      png_write_hIST(png_ptr, info_ptr->hist, info_ptr->num_palette);



   if (info_ptr->valid & 0x0100)
      png_write_oFFs(png_ptr, info_ptr->x_offset, info_ptr->y_offset,
          info_ptr->offset_unit_type);



   if (info_ptr->valid & 0x0400)
   ...
   if (info_ptr->valid & 0x4000)
      png_write_sCAL_s(png_ptr, (int)info_ptr->scal_unit,
          info_ptr->scal_s_width, info_ptr->scal_s_height);



   if (info_ptr->valid & 0x0080)
      png_write_pHYs(png_ptr, info_ptr->x_pixels_per_unit,
          info_ptr->y_pixels_per_unit, info_ptr->phys_unit_type);



   if (info_ptr->valid & 0x0200)
   {
   ...
   for (i = 0; i < info_ptr->num_text; i++)
   {
      ...
      else if (info_ptr->text[i].compression == 0)
      {
      ...
      else if (info_ptr->text[i].compression == -1)
      {
...
__attribute__((__malloc__)) png_structp png_create_write_struct_2 (png_const_charp user_png_ver, png_voidp error_ptr, png_error_ptr error_fn, png_error_ptr warn_fn, png_voidp mem_ptr, png_malloc_ptr malloc_fn, png_free_ptr free_fn)
...
# 1042 "../../../libs/png/pngwrite.c"
      if (png_ptr->row_buf != ((void *)0))
      {

         if ((png_ptr->do_filter & 0x10) && png_ptr->sub_row == ((void *)0))
         {
         ...
         if ((png_ptr->do_filter & 0x20) && png_ptr->up_row == ((void *)0))
         {
         ...
         if ((png_ptr->do_filter & 0x40) && png_ptr->avg_row == ((void *)0))
         {
            if (png_ptr->prev_row == ((void *)0))
            {
...
png_reset_filter_heuristics(png_structrp png_ptr)
{
   ...
   png_ptr->heuristic_method = 1;
   if (png_ptr->prev_filters != ((void *)0))
   {
      png_bytep old = png_ptr->prev_filters;
      png_ptr->prev_filters = ((void *)0);
      png_free(png_ptr, old);
   }
   if (png_ptr->filter_weights != ((void *)0))
   {
      png_uint_16p old = png_ptr->filter_weights;
      png_ptr->filter_weights = ((void *)0);
      png_free(png_ptr, old);
   }

   if (png_ptr->inv_filter_weights != ((void *)0))
   {
      png_uint_16p old = png_ptr->inv_filter_weights;
      png_ptr->inv_filter_weights = ((void *)0);
      png_free(png_ptr, old);
   }
   ...
   int num_weights)
{
   ...
   if (heuristic_method == 2)
   {
      ...
      if (png_ptr->filter_costs == ((void *)0))
      {
         png_ptr->filter_costs = (png_uint_16p)png_malloc(png_ptr,
             (png_uint_32)((sizeof (png_uint_16)) * 5));
      ...
      for (i = 0; i < 5; i++)
      {
         ...
         png_ptr->filter_costs[i] = (1<<(3));
      }


      png_ptr->heuristic_method = 2;


      return 1;
   }
   else if (heuristic_method == 0 ||
      heuristic_method == 1)
   {
      return 1;
   }
    ...
    png_const_doublep filter_costs)
{
   ...
   if (heuristic_method == 2)
   {
      ...
      for (i = 0; i < num_weights; i++)
      {
         ...
         {
            png_ptr->inv_filter_weights[i] =
                (png_uint_16)((1<<(8))*filter_weights[i]+.5);

            png_ptr->filter_weights[i] =
                (png_uint_16)((1<<(8))/filter_weights[i]+.5);
         }
      }
# 1279 "../../../libs/png/pngwrite.c"
      for (i = 0; i < 5; i++) if (filter_costs[i] >= 1.0)
      {
         png_ptr->inv_filter_costs[i] =
             (png_uint_16)((1<<(3)) / filter_costs[i] + .5);
      ...
      for (i = 0; i < num_weights; i++)
      {
         ...
         else
         {
            png_ptr->inv_filter_weights[i] = (png_uint_16)
               (((1<<(8))*filter_weights[i]+50000)/100000);

            png_ptr->filter_weights[i] = (png_uint_16)(((1<<(8))*
               100000 +(filter_weights[i]/2))/filter_weights[i]);
         }
      }
# 1334 "../../../libs/png/pngwrite.c"
      for (i = 0; i < 5; i++)
         if (filter_costs[i] >= 100000)
      {
         png_uint_32 tmp;
         ...
         tmp = (1<<(3))*100000 + (filter_costs[i]/2);
         tmp /= filter_costs[i];

         png_ptr->inv_filter_costs[i] = (png_uint_16)tmp;

         tmp = (1<<(3)) * filter_costs[i] + 50000;
         tmp /= 100000;
...
# 1599 "../../../libs/png/pngwrite.c"
      ...
      (0x1000|0x0800))
   {
      ...
      {
         if (transforms & 0x0800)
            png_app_error(png_ptr,
               "PNG_TRANSFORM_STRIP_FILLER: BEFORE+AFTER not supported");


         png_set_filler(png_ptr, 0, 1);
      }

      else if (transforms & 0x0800)
...
{
   png_structp png_ptr = png_create_write_struct("1.6.10", image,
...
png_write_image_16bit(png_voidp argument)
{
   ...
   const int channels = (image->format & 0x02U) ? 3 : 1;
   int aindex = 0;
   png_uint_32 y = image->height;

   if (image->format & 0x01U)
   {

         if (image->format & 0x20U)
         {
            aindex = -1;
            ++input_row;
            ++output_row;
...
# 1836 "../../../libs/png/pngwrite.c"
static png_byte
png_unpremultiply(png_uint_32 component, png_uint_32 alpha,
   png_uint_32 reciprocal )
{
# 1850 "../../../libs/png/pngwrite.c"
   ...
   else if (component > 0)
   {
      ...
      return (png_byte)((png_byte)((png_sRGB_base[(component)>>15] + ((((component)&0x7fff)*png_sRGB_delta[(component)>>15])>>12)) >> 8));
   }
   ...
   if (image->format & 0x01U)
   {
      ...
      while (y-- > 0)
      {
         ...
         while (out_ptr < row_end)
         {
            ...
            png_byte alphabyte = (png_byte)((((png_uint_32)(alpha) * 255) + 32895) >> 16);
            png_uint_32 reciprocal = 0;
            int c;
   ...
   else
   {
      ...
      while (y-- > 0)
      {
         ...
         while (out_ptr < row_end)
         {
            ...
            component *= 255;
            *out_ptr++ = (png_byte)((png_byte)((png_sRGB_base[(component)>>15] + ((((component)&0x7fff)*png_sRGB_delta[(component)>>15])>>12)) >> 8));
         }
...
png_image_set_PLTE(png_image_write_control *display)
{
   ...
   const int channels = (((format)&(0x02U|0x01U))+1);



      const int afirst = (format & 0x20U) != 0 &&
         (format & 0x01U) != 0;
      ...
      const int bgr = (format & 0x10U) ? 2 : 0;
   ...
   for (i=num_trans=0; i<entries; ++i)
   {
      ...
      if (format & 0x04U)
      {
         ...
         if (channels & 1)
         {
            if (channels >= 3)
            {
               palette[i].blue = (png_byte)((png_byte)((png_sRGB_base[(255 * entry[(2 ^ bgr)])>>15] + ((((255 * entry[(2 ^ bgr)])&0x7fff)*png_sRGB_delta[(255 * entry[(2 ^ bgr)])>>15])>>12)) >> 8))
                                   ;
               palette[i].green = (png_byte)((png_byte)((png_sRGB_base[(255 * entry[1])>>15] + ((((255 * entry[1])&0x7fff)*png_sRGB_delta[(255 * entry[1])>>15])>>12)) >> 8))
                           ;
               palette[i].red = (png_byte)((png_byte)((png_sRGB_base[(255 * entry[bgr])>>15] + ((((255 * entry[bgr])&0x7fff)*png_sRGB_delta[(255 * entry[bgr])>>15])>>12)) >> 8))
                             ;
            }

            else
               palette[i].blue = palette[i].red = palette[i].green =
                  (png_byte)((png_byte)((png_sRGB_base[(255 * *entry)>>15] + ((((255 * *entry)&0x7fff)*png_sRGB_delta[(255 * *entry)>>15])>>12)) >> 8));
         }
...
png_image_write_main(png_voidp argument)
{
   ...
   int colormap = (format & 0x08U) != 0;
   int linear = !colormap && (format & 0x04U) != 0;
   int alpha = !colormap && (format & 0x01U) != 0;
   ...
   if (display->row_stride == 0)
      display->row_stride = (((((*image).format)&0x08U)?1:((((*image).format)&(0x02U|0x01U))+1)) * (*image).width);


   if (format & 0x08U)
   {
      if (display->colormap != ((void *)0) && image->colormap_entries > 0)
      {
   ...
   if ((format & ~(png_uint_32)(0x02U | 0x04U |
         0x01U | 0x08U)) != 0)
