...
# 23 "../../../libs/jpeg/jquant2.c" 2
# 147 "../../../libs/jpeg/jquant2.c"
...

{
  ...
  boxlist = (boxptr) (*cinfo->mem->alloc_small)
    ((j_common_ptr) cinfo, 1, desired_colors * ((size_t) sizeof(box)));
...
# 645 "../../../libs/jpeg/jquant2.c"
static int
find_nearby_colors (j_decompress_ptr cinfo, int minc0, int minc1, int minc2,
      JSAMPLE colorlist[])
# 656 "../../../libs/jpeg/jquant2.c"
{
...

{
  ...
  table = (int *) (*cinfo->mem->alloc_small)
    ((j_common_ptr) cinfo, 1, (255*2+1) * ((size_t) sizeof(int)));
  table += 255;
...
start_pass_2_quant (j_decompress_ptr cinfo, boolean is_pre_scan)
{
  ...
  if (is_pre_scan) {
    ...
    if (cinfo->dither_mode == JDITHER_FS) {
      size_t arraysize = (size_t) ((cinfo->output_width + 2) *
       (3 * ((size_t) sizeof(FSERROR))));

      if (cquantize->fserrors == ((void *)0))
 cquantize->fserrors = (FSERRPTR) (*cinfo->mem->alloc_large)
   ((j_common_ptr) cinfo, 1, arraysize);
 ...
 init_error_limit(cinfo);
  ...
  (1<<6)*(1<<5) * ((size_t) sizeof(histcell)));
    }
...
jinit_2pass_quantizer (j_decompress_ptr cinfo)
{
  ...
  cquantize->histogram = (hist3d) (*cinfo->mem->alloc_small)
    ((j_common_ptr) cinfo, 1, (1<<5) * ((size_t) sizeof(hist2d)));
  for (i = 0; i < (1<<5); i++) {
  ...
  if (cinfo->enable_2pass_quant) {
    ...
    cquantize->sv_colormap = (*cinfo->mem->alloc_sarray)
      ((j_common_ptr) cinfo,1, (JDIMENSION) desired, (JDIMENSION) 3);
  ...
  if (cinfo->dither_mode == JDITHER_FS) {
    cquantize->fserrors = (FSERRPTR) (*cinfo->mem->alloc_large)
      ((j_common_ptr) cinfo, 1,
       (size_t) ((cinfo->output_width + 2) * (3 * ((size_t) sizeof(FSERROR)))));
