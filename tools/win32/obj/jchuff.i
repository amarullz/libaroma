...
# 20 "../../../libs/jpeg/jchuff.c" 2
# 1 "../../../libs/jpeg/jchuff.h" 1
# 27 "../../../libs/jpeg/jchuff.h"
typedef struct {
  unsigned int ehufco[256];
  char ehufsi[256];

} c_derived_tbl;
# 41 "../../../libs/jpeg/jchuff.h"
...
# 21 "../../../libs/jpeg/jchuff.c" 2
# 29 "../../../libs/jpeg/jchuff.c"
typedef struct {
  INT32 put_buffer;
  int put_bits;
  int last_dc_val[4];
} savable_state;
# 55 "../../../libs/jpeg/jchuff.c"
typedef struct {
...
start_pass_huff (j_compress_ptr cinfo, boolean gather_statistics)
{
 ...
 entropy->dc_count_ptrs[dctbl] = (long *)
   (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, 1,
      ...
      memset((void *)(entropy->dc_count_ptrs[dctbl]), 0, (size_t)(257 * ((size_t) sizeof(long))));
      if (entropy->ac_count_ptrs[actbl] == ((void *)0))
 entropy->ac_count_ptrs[actbl] = (long *)
   (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, 1,
          257 * ((size_t) sizeof(long)));
      memset((void *)(entropy->ac_count_ptrs[actbl]), 0, (size_t)(257 * ((size_t) sizeof(long))));

    } else {


      jpeg_make_c_derived_tbl(cinfo, 1, dctbl,
         & entropy->dc_derived_tbls[dctbl]);
      jpeg_make_c_derived_tbl(cinfo, 0, actbl,
...

{
  ...
  while (put_bits >= 8) {
    ...
    { *(state)->next_output_byte++ = (JOCTET) (c); if (--(state)->free_in_buffer == 0) if (! dump_buffer(state)) { return 0; } };
    if (c == 0xFF) {
      { *(state)->next_output_byte++ = (JOCTET) (0); if (--(state)->free_in_buffer == 0) if (! dump_buffer(state)) { return 0; } };
    }
    put_buffer <<= 8;
    put_bits -= 8;
  }
    ...
    c_derived_tbl *dctbl, c_derived_tbl *actbl)
{
  ...
  if (nbits > 10 +1)
    ((state->cinfo)->err->msg_code = (JERR_BAD_DCT_COEF), (*(state->cinfo)->err->error_exit) ((j_common_ptr) (state->cinfo)));
...
emit_restart (working_state * state, int restart_num)
{
  ...
  { *(state)->next_output_byte++ = (JOCTET) (0xFF); if (--(state)->free_in_buffer == 0) if (! dump_buffer(state)) { return 0; } };
  { *(state)->next_output_byte++ = (JOCTET) (0xD0 + restart_num); if (--(state)->free_in_buffer == 0) if (! dump_buffer(state)) { return 0; } };
   ...
   long dc_counts[], long ac_counts[])
{
  ...
  if (nbits > 10 +1)
    ((cinfo)->err->msg_code = (JERR_BAD_DCT_COEF), (*(cinfo)->err->error_exit) ((j_common_ptr) (cinfo)));
