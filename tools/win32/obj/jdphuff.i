...
# 47 "../../../libs/jpeg/jinclude.h" 2
# 66 "../../../libs/jpeg/jinclude.h"
...
# 67 "../../../libs/jpeg/jinclude.h" 2
# 19 "../../../libs/jpeg/jdphuff.c" 2
# 1 "../../../libs/jpeg/jpeglib.h" 1
# 26 "../../../libs/jpeg/jpeglib.h"
# 1 "../../../libs/jpeg/jmorecfg.h" 1
# 16 "../../../libs/jpeg/jmorecfg.h"
...
# 1180 "../../../libs/jpeg/jpeglib.h"
# 1 "../../../libs/jpeg/jpegint.h" 1
# 16 "../../../libs/jpeg/jpegint.h"
typedef enum {
 JBUF_PASS_THRU,
...
extern void jset_input_stream_position (j_decompress_ptr cinfo, int offset)
                                ;
extern void jset_input_stream_position_bit (j_decompress_ptr cinfo, int byte_offset, int bit_left, INT32 buf)
                                                              ;
...
# 1182 "../../../libs/jpeg/jpeglib.h" 2
# 20 "../../../libs/jpeg/jdphuff.c" 2
# 1 "../../../libs/jpeg/jdhuff.h" 1
# 26 "../../../libs/jpeg/jdhuff.h"
typedef struct {
...
extern void jpeg_make_d_derived_tbl
 (j_decompress_ptr cinfo, boolean isDC, int tblno, d_derived_tbl ** pdtbl)
                              ;
# 72 "../../../libs/jpeg/jdhuff.h"
typedef INT32 bit_buf_type;
# 83 "../../../libs/jpeg/jdhuff.h"
typedef struct {
...
# 155 "../../../libs/jpeg/jdhuff.h"
extern boolean jpeg_fill_bit_buffer
 (bitread_working_state * state, register bit_buf_type get_buffer, register int bits_left, int nbits)
                                         ;
# 200 "../../../libs/jpeg/jdhuff.h"
extern int jpeg_huff_decode
 (bitread_working_state * state, register bit_buf_type get_buffer, register int bits_left, d_derived_tbl * htbl, int min_bits)
                                                                  ;
# 21 "../../../libs/jpeg/jdphuff.c" 2
# 32 "../../../libs/jpeg/jdphuff.c"
typedef struct {
  unsigned int EOBRUN;
  int last_dc_val[4];
} savable_state;
# 56 "../../../libs/jpeg/jdphuff.c"
typedef struct {
...
start_pass_phuff_decoder (j_decompress_ptr cinfo)
{
 ...
 jpeg_make_d_derived_tbl(cinfo, 1, tbl,
    & entropy->derived_tbls[tbl]);
      }
    } else {
      tbl = compptr->ac_tbl_no;
      jpeg_make_d_derived_tbl(cinfo, 0, tbl,
         & entropy->derived_tbls[tbl]);
  ...
  entropy->pub.insufficient_data = 0;
...
decode_mcu_DC_first (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
  ...
  int blkn, ci;
  JBLOCKROW block;
  register bit_buf_type get_buffer; register int bits_left; bitread_working_state br_state;
  savable_state state;
  d_derived_tbl * tbl;
  jpeg_component_info * compptr;
 ...
 return 0;
  ...
  if (! entropy->pub.insufficient_data) {
    ...
    br_state.cinfo = cinfo; br_state.next_input_byte = cinfo->src->next_input_byte; br_state.bytes_in_buffer = cinfo->src->bytes_in_buffer; get_buffer = entropy->bitstate.get_buffer; bits_left = entropy->bitstate.bits_left;;
    ((state) = (entropy->saved));
    ...
    for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
      ...
      { register int nb, look; if (bits_left < 8) { if (! jpeg_fill_bit_buffer(&br_state,get_buffer,bits_left, 0)) {return 0;} get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; if (bits_left < 8) { nb = 1; goto label1; } } look = (((int) (get_buffer >> (bits_left - (8)))) & ((1<<(8))-1)); if ((nb = tbl->look_nbits[look]) != 0) { (bits_left -= (nb)); s = tbl->look_sym[look]; } else { nb = 8 +1; label1: if ((s=jpeg_huff_decode(&br_state,get_buffer,bits_left,tbl,nb)) < 0) { return 0; } get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; } };
      if (s) {
 { if (bits_left < (s)) { if (! jpeg_fill_bit_buffer(&(br_state),get_buffer,bits_left,s)) { return 0; } get_buffer = (br_state).get_buffer; bits_left = (br_state).bits_left; } };
 ...
 s = ((r) < (1<<((s)-1)) ? (r) + (((-1)<<(s)) + 1) : (r));
    ...
    cinfo->src->next_input_byte = br_state.next_input_byte; cinfo->src->bytes_in_buffer = br_state.bytes_in_buffer; entropy->bitstate.get_buffer = get_buffer; entropy->bitstate.bits_left = bits_left;
    ((entropy->saved) = (state));
  }
...
decode_mcu_AC_first (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
 ...
 { register int nb, look; if (bits_left < 8) { if (! jpeg_fill_bit_buffer(&br_state,get_buffer,bits_left, 0)) {return 0;} get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; if (bits_left < 8) { nb = 1; goto label2; } } look = (((int) (get_buffer >> (bits_left - (8)))) & ((1<<(8))-1)); if ((nb = tbl->look_nbits[look]) != 0) { (bits_left -= (nb)); s = tbl->look_sym[look]; } else { nb = 8 +1; label2: if ((s=jpeg_huff_decode(&br_state,get_buffer,bits_left,tbl,nb)) < 0) { return 0; } get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; } };
 r = s >> 4;
 s &= 15;
 if (s) {
   k += r;
   ...
   if (r == 15) {
     ...
     EOBRUN = 1 << r;
     if (r) {
       { if (bits_left < (r)) { if (! jpeg_fill_bit_buffer(&(br_state),get_buffer,bits_left,r)) { return 0; } get_buffer = (br_state).get_buffer; bits_left = (br_state).bits_left; } };
...
decode_mcu_DC_refine (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
 ...
 return 0;
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    { if (bits_left < (1)) { if (! jpeg_fill_bit_buffer(&(br_state),get_buffer,bits_left,1)) { return 0; } get_buffer = (br_state).get_buffer; bits_left = (br_state).bits_left; } };
...
decode_mcu_AC_refine (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
 ...
 return 0;
  ...
  if (! entropy->pub.insufficient_data) {
    ...
    if (EOBRUN == 0) {
      for (; k <= Se; k++) {
 { register int nb, look; if (bits_left < 8) { if (! jpeg_fill_bit_buffer(&br_state,get_buffer,bits_left, 0)) {goto undoit;} get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; if (bits_left < 8) { nb = 1; goto label3; } } look = (((int) (get_buffer >> (bits_left - (8)))) & ((1<<(8))-1)); if ((nb = tbl->look_nbits[look]) != 0) { (bits_left -= (nb)); s = tbl->look_sym[look]; } else { nb = 8 +1; label3: if ((s=jpeg_huff_decode(&br_state,get_buffer,bits_left,tbl,nb)) < 0) { goto undoit; } get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; } };
 r = s >> 4;
 s &= 15;
 if (s) {
   if (s != 1)
     ((cinfo)->err->msg_code = (JWRN_HUFF_BAD_CODE), (*(cinfo)->err->emit_message) ((j_common_ptr) (cinfo), -1));
   { if (bits_left < (1)) { if (! jpeg_fill_bit_buffer(&(br_state),get_buffer,bits_left,1)) { goto undoit; } get_buffer = (br_state).get_buffer; bits_left = (br_state).bits_left; } };
   if ((((int) (get_buffer >> (bits_left -= (1)))) & ((1<<(1))-1)))
     s = p1;
   else
     s = m1;
   ...
   if (r != 15) {
     EOBRUN = 1 << r;
     if (r) {
       { if (bits_left < (r)) { if (! jpeg_fill_bit_buffer(&(br_state),get_buffer,bits_left,r)) { goto undoit; } get_buffer = (br_state).get_buffer; bits_left = (br_state).bits_left; } };
       r = (((int) (get_buffer >> (bits_left -= (r)))) & ((1<<(r))-1));
       EOBRUN += r;
     }
     break;
        ...
        huffman_offset_data *offset)
{
 ...
 return;
  ...
  offset->bitstream_offset =
      (jget_input_stream_position(cinfo) << 5)
        ...
        huffman_offset_data offset)
{
 phuff_entropy_ptr entropy = (phuff_entropy_ptr) cinfo->entropy;
  ...
  unsigned int byte_offset = bitstream_offset >> 5;
  unsigned int bit_in_bit_buffer =
      bitstream_offset & ((1 << 5) - 1);
...
jinit_phuff_decoder (j_decompress_ptr cinfo)
{
  ...
  entropy = (phuff_entropy_ptr)
    ...
    ((size_t) sizeof(phuff_entropy_decoder)));
  ...
  cinfo->coef_bits = (int (*)[64])
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, 1,
    cinfo->num_components*64*((size_t) sizeof(int)));
