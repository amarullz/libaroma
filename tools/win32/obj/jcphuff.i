...

{
  ...
  while (put_bits >= 8) {
    ...
    { *(entropy)->next_output_byte++ = (JOCTET) (c); if (--(entropy)->free_in_buffer == 0) dump_buffer(entropy); };
    if (c == 0xFF) {
      { *(entropy)->next_output_byte++ = (JOCTET) (0); if (--(entropy)->free_in_buffer == 0) dump_buffer(entropy); };
    }
    put_buffer <<= 8;
    put_bits -= 8;
  }
...
emit_restart (phuff_entropy_ptr entropy, int restart_num)
{
  ...
  if (! entropy->gather_statistics) {
    flush_bits(entropy);
    { *(entropy)->next_output_byte++ = (JOCTET) (0xFF); if (--(entropy)->free_in_buffer == 0) dump_buffer(entropy); };
    { *(entropy)->next_output_byte++ = (JOCTET) (0xD0 + restart_num); if (--(entropy)->free_in_buffer == 0) dump_buffer(entropy); };
  }
...
jinit_phuff_encoder (j_compress_ptr cinfo)
{
  ...
  entropy = (phuff_entropy_ptr)
    (*cinfo->mem->alloc_small) ((j_common_ptr) cinfo, 1,
    ((size_t) sizeof(phuff_entropy_encoder)));
