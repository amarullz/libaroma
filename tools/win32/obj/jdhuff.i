...
extern int jpeg_huff_decode
 (bitread_working_state * state, register bit_buf_type get_buffer, register int bits_left, d_derived_tbl * htbl, int min_bits)
                                                                  ;
# 21 "../../../libs/jpeg/jdhuff.c" 2

static boolean process_restart (j_decompress_ptr cinfo);
# 32 "../../../libs/jpeg/jdhuff.c"
typedef struct {
  int last_dc_val[4];
} savable_state;
# 54 "../../../libs/jpeg/jdhuff.c"
typedef struct {
...
start_pass_huff_decoder (j_decompress_ptr cinfo)
{
  ...
  for (ci = 0; ci < cinfo->comps_in_scan; ci++) {
    ...
    jpeg_make_d_derived_tbl(cinfo, 1, dctbl,
       & entropy->dc_derived_tbls[dctbl]);
    jpeg_make_d_derived_tbl(cinfo, 0, actbl,
  ...
  for (blkn = 0; blkn < cinfo->blocks_in_MCU; blkn++) {
    ...
    if (compptr->component_needed) {
      entropy->dc_needed[blkn] = 1;

      entropy->ac_needed[blkn] = (compptr->DCT_scaled_size > 1);
    } else {
      entropy->dc_needed[blkn] = entropy->ac_needed[blkn] = 0;
    }
  }
...
# 351 "../../../libs/jpeg/jdhuff.c"
 ...
 ((cinfo)->err->msg_code = (JWRN_HIT_MARKER), (*(cinfo)->err->emit_message) ((j_common_ptr) (cinfo), -1));
 cinfo->entropy->insufficient_data = 1;
      }

      get_buffer <<= (32 -7) - bits_left;
      bits_left = (32 -7);
    }
  }
    ...
    d_derived_tbl * htbl, int min_bits)
{
  ...
  { if (bits_left < (l)) { if (! jpeg_fill_bit_buffer(&(*state),get_buffer,bits_left,l)) { return -1; } get_buffer = (*state).get_buffer; bits_left = (*state).bits_left; } };
  ...
  while (code > htbl->maxcode[l]) {
    code <<= 1;
    { if (bits_left < (1)) { if (! jpeg_fill_bit_buffer(&(*state),get_buffer,bits_left,1)) { return -1; } get_buffer = (*state).get_buffer; bits_left = (*state).bits_left; } };
...
decode_mcu (j_decompress_ptr cinfo, JBLOCKROW *MCU_data)
{
 ...
 for (k = 1; k < 64; k++) {
   { register int nb, look; if (bits_left < 8) { if (! jpeg_fill_bit_buffer(&br_state,get_buffer,bits_left, 0)) {return 0;} get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; if (bits_left < 8) { nb = 1; goto label2; } } look = (((int) (get_buffer >> (bits_left - (8)))) & ((1<<(8))-1)); if ((nb = actbl->look_nbits[look]) != 0) { (bits_left -= (nb)); s = actbl->look_sym[look]; } else { nb = 8 +1; label2: if ((s=jpeg_huff_decode(&br_state,get_buffer,bits_left,actbl,nb)) < 0) { return 0; } get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; } };
 ...
 for (k = 1; k < 64; k++) {
   { register int nb, look; if (bits_left < 8) { if (! jpeg_fill_bit_buffer(&br_state,get_buffer,bits_left, 0)) {return 0;} get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; if (bits_left < 8) { nb = 1; goto label3; } } look = (((int) (get_buffer >> (bits_left - (8)))) & ((1<<(8))-1)); if ((nb = actbl->look_nbits[look]) != 0) { (bits_left -= (nb)); s = actbl->look_sym[look]; } else { nb = 8 +1; label3: if ((s=jpeg_huff_decode(&br_state,get_buffer,bits_left,actbl,nb)) < 0) { return 0; } get_buffer = br_state.get_buffer; bits_left = br_state.bits_left; } };
