...
# 20 "../../../libs/freetype/src/base/ftbitmap.c" 2
...
# 21 "../../../libs/freetype/src/base/ftbitmap.c" 2

# 1 "../../../libs/freetype/include/freetype/ftbitmap.h" 1
# 33 "../../../libs/freetype/include/freetype/ftbitmap.h"

# 69 "../../../libs/freetype/include/freetype/ftbitmap.h"
  extern void
...
# 97 "../../../libs/freetype/include/freetype/ftbitmap.h"
  extern FT_Error
  FT_Bitmap_Copy( FT_Library library,
                  const FT_Bitmap *source,
                  FT_Bitmap *target);
# 138 "../../../libs/freetype/include/freetype/ftbitmap.h"
  extern FT_Error
  FT_Bitmap_Embolden( FT_Library library,
                      FT_Bitmap* bitmap,
                      FT_Pos xStrength,
                      FT_Pos yStrength );
# 178 "../../../libs/freetype/include/freetype/ftbitmap.h"
  extern FT_Error
  FT_Bitmap_Convert( FT_Library library,
                     const FT_Bitmap *source,
                     FT_Bitmap *target,
                     FT_Int alignment );
# 203 "../../../libs/freetype/include/freetype/ftbitmap.h"
  extern FT_Error
  FT_GlyphSlot_Own_Bitmap( FT_GlyphSlot slot );
# 227 "../../../libs/freetype/include/freetype/ftbitmap.h"
  extern FT_Error
...
# 23 "../../../libs/freetype/src/base/ftbitmap.c" 2
...
# 25 "../../../libs/freetype/src/base/ftbitmap.c" 2
                  ...
                  FT_Bitmap *target)
  {
    ...
    if ( target->buffer )
    {
      ...
      if ( target_size != size )
        (void)( ((target->buffer) = (ft_mem_qrealloc( memory, 1, (FT_Long)(target_size), (FT_Long)(size), (target->buffer), &error ))), error != 0 );
    }
    else
      (void)( ((target->buffer) = (ft_mem_qalloc( memory, (FT_Long)(size), &error ))), error != 0 );
    ...
    if ( !error )
    {
      ...
      *target = *source;
      ...
      if ( source_pitch_sign == target_pitch_sign )
        memcpy( target->buffer, source->buffer, (FT_Offset)(size) );
      else
      {
        ...
        {
          memcpy( t, s, (FT_Offset)((FT_Offset)(pitch) * sizeof ( *(t) )) );
                           ...
                           FT_UInt ypixels )
  {
    ...
    if ( ypixels == 0 && new_pitch <= pitch )
    {
      ...
      if ( bit_last < bit_width )
      {
        ...
        for ( ; count > 0; count--, line += pitch, end += pitch )
        {
          ...
          if ( write < end )
            memset( write, 0, (FT_Offset)(end - write) );
        }
      }
    ...
    if ( ( ((buffer) = (ft_mem_qrealloc( memory, (FT_Long)(bitmap->rows + ypixels), 0, (FT_Long)(new_pitch), ((void *)0), &error ))), error != 0 ) )
      return error;
    ...
    if ( bitmap->pitch > 0 )
    {
      ...
      for ( i = 0; i < bitmap->rows; i++ )
        memcpy( buffer + (FT_UInt)new_pitch * ( ypixels + i ), bitmap->buffer + (FT_UInt)pitch * i, (FT_Offset)(len) )
    ...
    else
    {
      ...
      for ( i = 0; i < bitmap->rows; i++ )
        memcpy( buffer + (FT_UInt)new_pitch * i, bitmap->buffer + (FT_UInt)pitch * i, (FT_Offset)(len) )
                      ...
                      FT_Pos yStrength )
  {
    ...
    if ( ( ( ( ((xStrength) + 32) & ~(__typeof__ ((xStrength) + 32))63 ) >> 6 ) > 2147483647 ) ||
         ( ( ( ((yStrength) + 32) & ~(__typeof__ ((yStrength) + 32))63 ) >> 6 ) > 2147483647 ) )
      return FT_Err_Invalid_Argument;

    xstr = (FT_Int)( ((xStrength) + 32) & ~(__typeof__ ((xStrength) + 32))63 ) >> 6;
    ystr = (FT_Int)( ((yStrength) + 32) & ~(__typeof__ ((yStrength) + 32))63 ) >> 6;
  ...
  ft_gray_for_premultiplied_srgb_bgra( const FT_Byte* bgra )
  {
    ...
    if ( !a )
      return 0;
# 455 "../../../libs/freetype/src/base/ftbitmap.c"
    l = ( 4732UL * bgra[0] * bgra[0] +
          46871UL * bgra[1] * bgra[1] +
          13933UL * bgra[2] * bgra[2] ) >> 16;
# 472 "../../../libs/freetype/src/base/ftbitmap.c"
    ...
    case FT_PIXEL_MODE_BGRA:
      {
        ...
        if ( target_pitch > 0 &&
             (FT_ULong)target->rows > (2147483647L * 2UL + 1UL) / (FT_ULong)target_pitch )
          return FT_Err_Invalid_Argument;

        if ( target->rows * (FT_ULong)target_pitch > old_size &&
             ( ((target->buffer) = (ft_mem_qrealloc( memory, 1, (FT_Long)(old_size), (FT_Long)(target->rows * (FT_UInt)target_pitch), (target->buffer), &error ))), error != 0 )
                                                                           )
          return error;
    ...
    case FT_PIXEL_MODE_LCD_V:
      {
        ...
        for ( i = source->rows; i > 0; i-- )
        {
          memcpy( t, s, (FT_Offset)((FT_Offset)(width) * sizeof ( *(t) )) );
