...
static int gz_skip (gz_statep, off_t);
    ...
    gz_statep state;
{
    ...
    if (state->size == 0) {
        ...
        if (inflateInit2_((&(state->strm)), (15 + 16), "1.2.8", (int)sizeof(z_stream)) != 0) {
            free(state->out);
            free(state->in);
            state->size = 0;
            gz_error(state, (-4), "out of memory");
