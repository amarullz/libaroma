...
# 232 "d:\\devtools\\linaro\\arm-linux-gnueabihf\\libc\\usr\\include\\dirent.h" 3 4
...
# 233 "d:\\devtools\\linaro\\arm-linux-gnueabihf\\libc\\usr\\include\\dirent.h" 2 3 4
...
# 147 "d:\\devtools\\linaro\\arm-linux-gnueabihf\\libc\\usr\\include\\limits.h" 3 4
  ...
  while (*p != '\0') {
    ...
    *p = '\0';
    ...
    if (ds == DILLEGAL) {
      ...
      if (timestamp != ((void *)0) && utime(cpath, timestamp)) {
        free(cpath);
        return -1;
      }
    }
...
dirUnlinkHierarchy(const char * path) {
  ...
  if (fail || (*__errno_location ()) < 0) {
    int save = (*__errno_location ());
    closedir(dir);
...
dirSetHierarchyPermissions(const char * path,
  ...
  if (chown(path, uid, gid) ||
      chmod(path, ((((st.st_mode)) & 0170000) == (0040000)) ? dirMode : fileMode)) {
    return -1;
  }
  ...
  if (((((st.st_mode)) & 0170000) == (0040000))) {
    ...
    while ((*__errno_location ()) == 0 && (de = readdir(dir)) != ((void *)0)) {
      if (!strcmp(de->d_name, "..") || !strcmp(de->d_name, ".")) {
        continue;
      }
