...
struct _LIBAROMA_MSGQUEUE{
  LIBAROMA_STACKP input;
  LIBAROMA_STACKP queue;
  LIBAROMA_THREAD input_thread;
};
...
static LIBAROMA_MSGQUEUEP _libaroma_msgqueue=((void *)0);
...
static pthread_mutex_t _libaroma_msgqueue_mutex;
static pthread_cond_t _libaroma_msgqueue_cond;
...
byte libaroma_msg_init() {
  if (_libaroma_msgqueue != ((void *)0)) {
    fprintf(libaroma_debug_output(), "E/%s: ",libaroma_debug_tag()); fprintf(libaroma_debug_output(), "message instance already initialized"); fprintf(libaroma_debug_output(), "%s\n","");;
    return 0;
  }
  ...
  _libaroma_msgqueue->queue = libaroma_stack(((void *)0));

  _libaroma_msgqueue->input = libaroma_stack(((void *)0));

  _libaroma_msgqueue_isrun = 1;

  pthread_create(&_libaroma_msgqueue->input_thread,((void *)0),_libaroma_msgqueue_hid_thread,((void *)0))
                                                                          ;
...
void libaroma_msg_clean_hid() {
  if (_libaroma_msgqueue == ((void *)0)) {
    fprintf(libaroma_debug_output(), "E/%s: ",libaroma_debug_tag()); fprintf(libaroma_debug_output(), "message instance uninitialized"); fprintf(libaroma_debug_output(), "%s\n","");;
    return;
  }
