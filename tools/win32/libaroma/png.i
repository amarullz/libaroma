...
# 27 "../../../src/aroma/graph/png.c" 2
# 1 "../../../libs/png/png.h" 1
# 457 "../../../libs/png/png.h"
# 1 "../../../libs/png/pnglibconf.h" 1
# 458 "../../../libs/png/png.h" 2
...
LIBAROMA_CANVASP libaroma_png_ex(
  ...
  png_ptr = png_create_read_struct("1.6.10", ((void *)0), ((void *)0), ((void *)0));
  ...
  if ((color_type == 0)||
    ...
    png_read_update_info(png_ptr, info_ptr);
           ...
           (channels == 3 && color_type == (2)) ||
           (channels == 4 && color_type == (2 | 4))
         )) ||
        (channels == 1 && color_type == (2 | 1))
       )) {
  ...
  cv = libaroma_canvas_new_ex(pcv_w,pcv_h,(pcv_c==4)?1:0,hicolor,((void *)0))
                                            ;
  ...
  if (pcv_c == 4) {
    ...
    if (hicolor) {
      for (y = 0; y < pcv_h; ++y) {
        ...
        png_read_row(png_ptr, row_data, ((void *)0));
...
byte libaroma_png9p_draw(
  ...
  LIBAROMA_PNG9 v;
  if (!libaroma_png9p(src, &v, (padding!=((void *)0))?1:0)) {
    ;
    return 0;
  }
  ...
  int RW = src->w - 1 - RX;
  int BH = src->h - 1 - BY;
  int drawX = (((((v.x - 1) * dp_scale) >> 8)<(maxW))?(((v.x - 1) * dp_scale) >> 8):(maxW));
  int drawY = (((((v.y - 1) * dp_scale) >> 8)<(maxW))?(((v.y - 1) * dp_scale) >> 8):(maxW));
  int drawW = (((((v.x + RW) * dp_scale) >> 8)<(maxW))?(((v.x + RW) * dp_scale) >> 8):(maxW));
  int drawH = (((((v.y + BH) * dp_scale) >> 8)<(maxH))?(((v.y + BH) * dp_scale) >> 8):(maxH));
  int drawRW = ((((RW * dp_scale) >> 8)<(maxW))?((RW * dp_scale) >> 8):(maxW));
  int drawBH = ((((BH * dp_scale) >> 8)<(maxH))?((BH * dp_scale) >> 8):(maxH));
...
byte libaroma_png_save(
  ...
  png_structp png_ptr = ((void *)0);
  volatile png_infop info_ptr = ((void *)0);
  ...
  png_ptr = png_create_write_struct("1.6.10", ((void *)0), ((void *)0), ((void *)0));
  if (!png_ptr) {
    ;
    goto finalize;
  }
  ...
  title_text.text = libaroma_info(0x3);
  png_set_text(png_ptr, info_ptr, &title_text, 1);

  title_text.key = "Description";
  title_text.text = "captured from " "libaroma" " canvas";
  png_set_text(png_ptr, info_ptr, &title_text, 1);

  title_text.key = "Copyright";
  title_text.text = libaroma_info(0x7);
  png_set_text(png_ptr, info_ptr, &title_text, 1);

  title_text.key = "Software";
  title_text.text = libaroma_info(0x8);
  png_set_text(png_ptr, info_ptr, &title_text, 1);
    ...
    for (y = 0 ; y < c->h ; y++) {
      ...
      for (x = 0 ; x < c->w ; x++) {
        ...
        if (c->hicolor) {
          ...
          row[dpos + 2] = libaroma_color_hi_r(libaroma_color_b(px));
        }
        if (c->alpha) {
  ...
  result = 1;
  fprintf(libaroma_debug_output(), "N/%s: ",libaroma_debug_tag()); fprintf(libaroma_debug_output(), "libaroma_png_save \"%s\"", filename); fprintf(libaroma_debug_output(), "\n");;
finalize:
