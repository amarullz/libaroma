# 1 "../../../src/aroma/controls/listitem/listitem_image.c"
# 1 "<command-line>"
# 1 "d:\\devtools\\linaro\\arm-linux-gnueabihf\\libc\\usr\\include\\stdc-predef.h" 1 3 4
# 1 "<command-line>" 2
# 1 "../../../src/aroma/controls/listitem/listitem_image.c"
# 26 "../../../src/aroma/controls/listitem/listitem_image.c"
...
# 27 "../../../src/aroma/controls/listitem/listitem_image.c" 2
...
# 28 "../../../src/aroma/controls/listitem/listitem_image.c" 2
    ...
    case 0x10:
      {
        if (item->flags&0x0002){
          int ysc=libaroma_ctl_scroll_get_scroll(ctl,((void *)0));
...
void _libaroma_listitem_image_draw(
  ...
  if (!(state&0x10)){
    ...
    if (mi->ready_image){
      if (flags&0x1000){
        if ((mi->ready_image->w!=cv->w)||(mi->ready_image->h!=cv->h*2)){
          libaroma_canvas_free_ex1(&(mi->ready_image), 0);
          mi->ready_image=((void *)0);
        }
      }
      else{
    ...
    if (mi->ready_image==((void *)0)){
      if (flags&0x1000){
        mi->ready_image=libaroma_canvas_new_ex(cv->w,cv->h*2,0,0,((void *)0));
        mi->paralax_last_y=-1;
        _libaroma_listitem_image_message(
          ctl, item, 0x10,0,0,0);
      }
      else{
        mi->ready_image=libaroma_canvas_new_ex(cv->w,cv->h,0,0,((void *)0));
      }
      libaroma_draw_rect(
        mi->ready_image,0,0,
      ...
      if (mi->ready_image){
        if (flags&0x0100){
          if (flags&0x0200){
            ...
            LIBAROMA_CANVASP tc=libaroma_canvas_new_ex(draw_w,draw_h,0,0,((void *)0));
            if (tc){
              ...
              libaroma_draw_ex2(mi->ready_image,tc,0,0,draw_x,draw_y,mi->ready_image->w,mi->ready_image->h,0,0xff,0,0)
          ...
          libaroma_draw_ex2(mi->ready_image,mi->image,(mi->ready_image->w>>1)-(mi->image->w>>1),(mi->ready_image->h>>1)-(mi->image->h>>1),0,0,mi->image->w,mi->image->h,1,0xff,0,0)
    ...
    if (mi->ready_image){
      if (flags&0x1000){
        libaroma_draw_ex2(cv,mi->ready_image,0,0,0,item->h-mi->paralax_y,cv->w,cv->h,0,0xff,0,0)
        ...
        libaroma_draw_ex2(cv,mi->ready_image,0,0,0,0,mi->ready_image->w,mi->ready_image->h,0,0xff,0,0);
      }
    }
    int min_pushed=0;
    word selcolor = is_dark?libaroma_rgb_to16(0xffffff):libaroma_rgb_to16(0x000000);
...
void _libaroma_listitem_image_release_internal(_LIBAROMA_LISTITEM_IMAGEP mi,
  ...
  if (mi->image){
    if ((flags&0x0800)||
      (!(flags&0x0400))){
      libaroma_canvas_free_ex1(&(mi->image), 0);
    }
  }
  if (mi->ready_image){
    libaroma_canvas_free_ex1(&(mi->ready_image), 0);
