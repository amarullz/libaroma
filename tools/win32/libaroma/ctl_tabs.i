...
# 27 "../../../src/aroma/controls/ctl_tabs.c" 2
...
# 28 "../../../src/aroma/controls/ctl_tabs.c" 2
...
int _libaroma_ctl_tabs_indicator_x(LIBAROMA_CONTROLP ctl, int pageno){
  ...
  if (ctl->handler!=&_libaroma_ctl_tabs_handler){ return 0; } _LIBAROMA_CTL_TABSP me = (_LIBAROMA_CTL_TABSP) ctl->internal
...
void _libaroma_ctl_tabs_pager_onscroll(
  ...
  if (ctl->handler!=&_libaroma_ctl_tabs_handler){ return ; } _LIBAROMA_CTL_TABSP me = (_LIBAROMA_CTL_TABSP) ctl->internal
...
int _libaroma_ctl_tabs_pager_pos_x(LIBAROMA_CONTROLP ctl, int x){
  if (ctl->handler!=&_libaroma_ctl_tabs_handler){ return -1; } _LIBAROMA_CTL_TABSP me = (_LIBAROMA_CTL_TABSP) ctl->internal
...
void _libaroma_ctl_tabs_internal_draw(LIBAROMA_CONTROLP ctl){
  ...
  int max_w = (show_w-sum_w) / (((1)>((((3)<(page_n))?(3):(page_n))))?(1):((((3)<(page_n))?(3):(page_n))));
  ...
  if (page_n>0){
    ...
    for (i=0;i<page_n;i++){
      ...
      if (page_n>3){
        me->textw[i]=
          libaroma_text_line_info(texts[i],0,0x2);
      }
      else{
    ...
    int ch = ctl->h;
    rest_canvas = libaroma_canvas_new_ex(cw,ch,0,0,((void *)0));
    push_canvas = libaroma_canvas_new_ex(cw,ch,0,0,((void *)0));
    ...
    for (i=0;i<page_n;i++){
      ...
      libaroma_text_draw_ex(rest_canvas,texts[i],xpos+itx,y,0,0,0,10,0,resttxt_color,0,0,0)

       ;
      libaroma_text_draw_ex(push_canvas,texts[i],xpos+itx,y,0,0,0,10,0,pushtxt_color,0,0,0)
...
void _libaroma_ctl_tabs_req_internal_draw(LIBAROMA_CONTROLP ctl){
  LIBAROMA_THREAD pp;
  pthread_create(&pp,((void *)0),_libaroma_ctl_tabs_req_internal_draw_thread,(void *) ctl)
                                                                 ;
  pthread_detach(pp);
}
...
void _libaroma_ctl_tabs_draw(
  ...
  if ((me->change_start>1)&&(me->change_state<1)){
    if (!me->change_dc){
      me->change_dc = libaroma_canvas_new_ex(c->w,c->h,0,0,((void *)0));
      if (me->change_dc){
        libaroma_draw_ex2(me->change_dc,c,0,0,0,0,c->w,c->h,0,0xff,0,0);
      }
    }
  }
  else if (me->change_dc){
    libaroma_canvas_free_ex1(&(me->change_dc), 0);
    me->change_dc=((void *)0);
  }
  ...
  int aw = me->active_w;
  libaroma_draw_ex2(c,me->rest_canvas,0,0,me->draw_x,0,c->w,c->h,0,0xff,0,0)
  ...
  if (me->active_page>=0){
    ...
    if (touched_x>=0){
      ...
      if ((xstart<c->w)&&(xend>0)){
        libaroma_draw_ex2(c,me->push_canvas,xstart,0,touched_x,0,touched_w,c->h,0,0xff,0,0)
  ...
  if (me->touched_id>=0){
    ...
    if (touched_x>=0){
      ...
      LIBAROMA_CANVASP tc = libaroma_canvas_new_ex(touched_w,cc->h,0,0,((void *)0));
      libaroma_draw_ex2(tc,cc,0,0,0,0,cc->w,cc->h,0,0xff,0,0);
      int ripple_i = 0;
      int ripple_p = 0;
      while(libaroma_ripple_loop(&me->ripple,&ripple_i,&ripple_p)){
        ...
        if (libaroma_ripple_calculation(
          ...
          libaroma_draw_circle(
            tc,me->selcolor,x,y,size,(((0)>(ripple_opacity-(ripple_opacity*0.5)))?(0):(ripple_opacity-(ripple_opacity*0.5)))
          );
          libaroma_draw_ex2(c,tc,touched_x-me->draw_x,0,0,0,tc->w,tc->h,0,0xff,0,0)
      ...
      libaroma_canvas_free_ex1(&(cc), 0);
      libaroma_canvas_free_ex1(&(tc), 0);
    }
  }
  ...
  if ((me->change_start>1)&&(me->change_state<1)){
    if (me->change_dc){
      ...
      libaroma_draw_ex2(c,me->change_dc,0,0,0,0,me->change_dc->w,me->change_dc->h,0,0xff * (1-xstate),0,0)
...
void _libaroma_ctl_tabs_destroy(
  ...
  if (me->controller.pager){
    libaroma_ctl_pager_set_controller(me->controller.pager, ((void *)0));
  }
...
byte libaroma_ctl_tabs_set_texts(LIBAROMA_CONTROLP ctl,
  ...
  for (i=0;i<textn;i++){
    me->texts[i]=(__extension__ (__builtin_constant_p (texts[i]) && ((size_t)(const void *)((texts[i]) + 1) - (size_t)(const void *)(texts[i]) == 1) ? (((const char *) (texts[i]))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (texts[i]) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void *)0)) __retval = (char *) memcpy (__retval, texts[i], __len); __retval; })) : __strdup (texts[i])));
  }
  me->textn=textn;
  omp_unset_nest_lock(&(me->mutex));
  if (update_now){
