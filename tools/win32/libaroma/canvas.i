...
LIBAROMA_CANVASP libaroma_canvas_new_ex(
  ...
  if (shmemname != ((void *)0)) {
    ...
    LIBAROMA_CANVAS_SHMEM_HEADERP csh_head = ((void *)0);
    bytep mem = ((void *)0);


    if (shmemname[0]=='@'){
      snprintf(nm, 0xff,
        "%s%s", "/tmp/.libaromashm-", shmemname+1);
    }
    else{
      snprintf(nm, 0xff,
        "%s", shmemname);
    ...
    if (fd < 0) {
      ...
      ;
      mem = (bytep) mmap(0, sz, 0x1 | 0x2, 0x01, fd, 0);
      close(fd);
      if (mem == ((void *) -1)) {
        ;
        return ((void *)0);
    ...
    csh_mem->mmap = mem;
    csh_mem->sz = sz;
    snprintf(csh_mem->name, 0xff, "%s", nm);
    ...
    c->data = (wordp)
      (mem + head_sz);
    c->alpha = (bytep)
      (csh_head->alpha?(mem+head_sz+data_sz):((void *)0));
    c->hicolor = (bytep)
      (csh_head->hicolor?(mem+head_sz+data_sz+alpha_sz):((void *)0));
...
LIBAROMA_CANVASP libaroma_canvas_dup(LIBAROMA_CANVASP c){
  ...
  byte alpha=(c->alpha!=((void *)0))?1:0;
  byte hicolor=(c->hicolor!=((void *)0))?1:0;
