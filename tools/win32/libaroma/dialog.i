...
void libaroma_dialog_updatebg(LIBAROMA_WINDOWP win, LIBAROMA_CANVASP c){
  ...
  libaroma_draw_ex2(c,cdata->bg,0,0,0,0,cdata->bg->w,cdata->bg->h,0,0xff,0,0);
  ...
  word bgcolor = libaroma_colorget(((void *)0), win)->dialog_bg;


  libaroma_gradient_ex1(c,cdata->x,cdata->y,cdata->w,cdata->h,bgcolor,bgcolor,libaroma_dp(2),0x1111,0xff,0xff,0)
...
LIBAROMA_WINDOWP libaroma_dialog_window(
  ...
  cdata->bg=libaroma_canvas_new_ex(libaroma_wm()->w,libaroma_wm()->h,0,0,((void *)0));
  LIBAROMA_CANVASP wmcv=libaroma_wm_canvas(0,0,libaroma_wm()->w,libaroma_wm()->h);

  if (flags&0x01){
    libaroma_draw_ex2(cdata->bg,wmcv,0,0,0,0,wmcv->w,wmcv->h,0,0x8F,0,0);
  }
  else{
    libaroma_draw_ex2(cdata->bg,wmcv,0,0,0,0,wmcv->w,wmcv->h,0,0xff,0,0);

  }
  libaroma_canvas_free_ex1(&(wmcv), 0);


  LIBAROMA_WINDOWP win = libaroma_window(((void *)0),0,0,libaroma_wm()->w,libaroma_wm()->h);
...
byte libaroma_dialog_free(
  ...
  if (win){
    if (win->client_data){
      ...
      libaroma_canvas_free_ex1(&(cdata->bg), 0);
...
int libaroma_dialog_confirm(
  ...
  word title_color = libaroma_colorget(((void *)0), win)->dialog_primary_text;
  word msg_color = libaroma_colorget(((void *)0), win)->dialog_secondary_text;
  word btncolor = libaroma_colorget(((void *)0), win)->dialog_primary_text;
  byte button_style = 0;
  if (flags&0x04){
    btncolor = libaroma_colorget(((void *)0), win)->accent;
    button_style=0x2;
  }
  ...
  libaroma_window_anishow(win,7,200);
...
int libaroma_dialog_list(
  ...
  LIBAROMA_CONTROLP listc=libaroma_ctl_list(
    ...
    10,
    0, 0,
    libaroma_colorget(((void *)0),win)->control_bg,
    0x80
  );

  LIBAROMA_CTL_LIST_ITEMP selitem=((void *)0);
  int u;
  for (u=0;u<24;u++){
    char extraText[100];
