...
# 27 "../../../src/aroma/controls/ctl_button.c" 2
...
# 28 "../../../src/aroma/controls/ctl_button.c" 2
...
void _libaroma_ctl_button_internal_draw(LIBAROMA_CONTROLP ctl){
  ...
  if (ctl->window==((void *)0)){
    return;
  }

  omp_set_nest_lock(&(me->mutex));
  LIBAROMA_CANVASP rest_canvas=((void *)0);
  LIBAROMA_CANVASP push_canvas=((void *)0);

  byte is_disabled = (me->style&0x4)?1:0;
  rest_canvas = libaroma_canvas_new_ex(ctl->w,ctl->h,0,0,((void *)0));
  if (!is_disabled){
    push_canvas = libaroma_canvas_new_ex(ctl->w,ctl->h,0,0,((void *)0));
  }
  LIBAROMA_CANVASP bg = libaroma_canvas_new_ex(ctl->w,ctl->h,0,0,((void *)0));
  ...
  byte is_circle=(me->style&0x8)?1:0;
  ...
  if (me->style&0x2) {
    ...
    me->isdark = libaroma_color_isdark(libaroma_colorget(ctl,((void *)0))->window_bg);
  }
  word push_color = me->isdark?libaroma_rgb_to16(0xffffff):libaroma_rgb_to16(0x000000);
  word text_color = me->isdark?0xffff:0;
  word rest_text_color = text_color;
  byte push_opa = me->isdark?50:30;


  LIBAROMA_CANVASP btnmask = libaroma_canvas_new_ex(iw,ih,1,0,((void *)0));
  libaroma_canvas_setcolor(btnmask,0,0);
  libaroma_gradient_ex1(btnmask,0,0,ctl->w-libaroma_dp(is_circle?12:8),ctl->h-libaroma_dp(12),0,0,is_circle?(((ctl->w)>(ctl->h))?(ctl->w):(ctl->h)):libaroma_dp(2),0x1111,0xff,0xff,0)
  ...
  if (me->style&0x1){
    ...
    libaroma_draw_zshadow(
      rest_canvas, btnmask, ix, iy, (me->style&0x2)?2:1
    );
  ...
  if (me->style&0x2) {
    if (is_disabled){
      libaroma_draw_ex2(btnmask,bg,0,0,ix,iy,btnmask->w,btnmask->h,0,0xff,0,0);
    ...
    if (me->style&0x1){
      libaroma_draw_ex2(rest_canvas,btnmask,ix,iy,0,0,btnmask->w,btnmask->h,1,0xff,0,0);
    }
    else{
    ...
    if (!is_disabled){
      ...
      libaroma_draw_ex2(push_canvas,btnmask,ix,iy,0,0,btnmask->w,btnmask->h,1,0xff,0,0);
    }
  }
  else{
    if (me->style&0x1){
  ...
  libaroma_text_draw_ex(rest_canvas,textp,libaroma_dp(8)+ix,y,0,0,0,10,0,rest_text_color,0,0,0)

   ;

  if (!is_disabled){
    libaroma_text_draw_ex(push_canvas,textp,libaroma_dp(8)+ix,y,0,0,0,0,0,0,0,0,0);
  }
  else{
    libaroma_draw_ex2(rest_canvas,bg,0,0,0,0,ctl->w,ctl->h,0,me->isdark?171:189,0,0)
                         ;
  }
  libaroma_text_free(textp);


  libaroma_canvas_free_ex1(&(bg), 0);
  libaroma_canvas_free_ex1(&(btnmask), 0);
  ...
  if (rc!=((void *)0)){
    libaroma_canvas_free_ex1(&(rc), 0);
  }
  if (pc!=((void *)0)){
    libaroma_canvas_free_ex1(&(pc), 0);
  }
  me->forcedraw=1;
...
void _libaroma_ctl_button_req_internal_draw(LIBAROMA_CONTROLP ctl){
  LIBAROMA_THREAD pp;
  pthread_create(&pp,((void *)0),_libaroma_ctl_button_req_internal_draw_thread,(void *) ctl)
                                                                   ;
  pthread_detach(pp);
}
...
void _libaroma_ctl_button_draw(
  ...
  if (ctl->handler!=&_libaroma_ctl_button_handler){ return ; } _LIBAROMA_CTL_BUTTONP me = (_LIBAROMA_CTL_BUTTONP) ctl->internal

   ;
  omp_set_nest_lock(&(me->mutex));
  if ((me->rest_canvas==((void *)0))||(me->forcedraw==2)){
    omp_unset_nest_lock(&(me->mutex));
    _libaroma_ctl_button_internal_draw(ctl);
    omp_set_nest_lock(&(me->mutex));
    if (me->rest_canvas==((void *)0)){
      omp_unset_nest_lock(&(me->mutex));
      return;
    }
  }
  ...
  if (me->style&0x4){
    libaroma_draw_ex2(c,me->rest_canvas,0,0,0,0,me->rest_canvas->w,me->rest_canvas->h,0,0xff,0,0);
  }
  else{
    int ripple_i = 0;
    int ripple_p = 0;
    libaroma_draw_ex2(c,me->rest_canvas,0,0,0,0,me->rest_canvas->w,me->rest_canvas->h,0,0xff,0,0);
    if (me->push_canvas!=((void *)0)){
      byte is_circle=(me->style&0x8)?1:0;
      while(libaroma_ripple_loop(&me->ripple,&ripple_i,&ripple_p)){
        ...
        if (libaroma_ripple_calculation(
          ...
          &x, &y, &size,ripple_p
            ...
            if (me->push_canvas!=((void *)0)){
              libaroma_draw_ex2(c,me->push_canvas,0,0,0,0,me->push_canvas->w,me->push_canvas->h,0,push_opacity*2,0,0);
            }
            word dcolor = me->isdark?libaroma_rgb_to16(0xffffff):0;
            byte push_opa = (byte) (
              (((int) ripple_opacity) * (me->isdark?64:30)) >> 8
            );
            if(is_circle){
              LIBAROMA_CANVASP rdc = libaroma_canvas_new_ex(c->w,c->h,0,0,((void *)0));
              if (rdc){
                libaroma_draw_ex2(rdc,c,0,0,0,0,c->w,c->h,0,0xff,0,0);
                libaroma_draw_circle(
                  rdc, dcolor, x, y, size, push_opa
                );
                int cntx=rdc->w>>1;
                int cnty=rdc->h>>1;
                int szxy=(((rdc->w)<(rdc->h))?(rdc->w):(rdc->h))-libaroma_dp(12);
...
byte _libaroma_ctl_button_thread(LIBAROMA_CONTROLP ctl) {
  ...
  if (ctl->handler!=&_libaroma_ctl_button_handler){ return 0; } _LIBAROMA_CTL_BUTTONP me = (_LIBAROMA_CTL_BUTTONP) ctl->internal
...
void _libaroma_ctl_button_destroy(
  ...
  omp_set_nest_lock(&(me->mutex));
  if (me->rest_canvas!=((void *)0)){
    libaroma_canvas_free_ex1(&(me->rest_canvas), 0);
    me->rest_canvas=((void *)0);
  }
  if (me->push_canvas!=((void *)0)){
    libaroma_canvas_free_ex1(&(me->push_canvas), 0);
    me->push_canvas=((void *)0);
  }
  if (me->text!=((void *)0)){
    free(me->text);
  }
...
LIBAROMA_CONTROLP libaroma_ctl_button(
  ...
  me->text = (__extension__ (__builtin_constant_p (text) && ((size_t)(const void *)((text) + 1) - (size_t)(const void *)(text) == 1) ? (((const char *) (text))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (text) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void *)0)) __retval = (char *) memcpy (__retval, text, __len); __retval; })) : __strdup (text)));
