...
# 27 "../../../src/aroma/graph/draw/gradient.c" 2
...
bytep _libaroma_gradient_corner(
  ...
  for (i = 1; i <= r; i++) {
    float w = sqrt(sz - i * i);
    int fw = (int) floor(w);
    byte t = (byte) (((round((w - ((float) fw)) * 0xff))<(0xff))?(round((w - ((float) fw)) * 0xff)):(0xff));
    int idx = ((r - i) * r) + (r - fw - 1);
    out[idx] = (byte) (((t)>(out[idx]))?(t):(out[idx]));
    idx = ((r - fw - 1) * r) + (r - i);
    out[idx] = (byte) (((t)>(out[idx]))?(t):(out[idx]));
...
static inline void _libaroma_gradient_draw_rounded(
    wordp line_mem, bytep line_alpha,
    wordp roundTmp,
    bytep roundData, int roundSize, byte isRight, int y) {
  int i;
  if (line_alpha != ((void *)0)) {
    if (isRight) {
      for (i = 0; i < roundSize; i++) {
        line_alpha[i] = (((line_alpha[i] + roundData[y * roundSize + (roundSize - i) - 1] - 0xff)>(0))?(line_alpha[i] + roundData[y * roundSize + (roundSize - i) - 1] - 0xff):(0))
                                                                   ;
      }
    }
    else {
      for (i = 0; i < roundSize; i++) {
        line_alpha[i] = (((line_alpha[i] + roundData[y * roundSize + i] - 0xff)>(0))?(line_alpha[i] + roundData[y * roundSize + i] - 0xff):(0))
                                                 ;
      }
    }
  }
...
# 175 "../../../src/aroma/graph/draw/gradient.c"
#pragma omp parallel for
# 175 "../../../src/aroma/graph/draw/gradient.c"


  for (_Y = 0; _Y < h; _Y++) {
    bytep line_alpha = ((void *)0);
    wordp roundTmp = ((void *)0);
    if (roundData!=((void *)0)){
      roundTmp = (wordp) malloc(roundSize * 2 * 2);
    }
    int ypos = y + _Y;
    byte cR=0,cG=0,cB=0;
# 197 "../../../src/aroma/graph/draw/gradient.c"
    word intensity = (_Y * 0x100) / h;
    word r_intensity = 0x100 - intensity;
    if (!samecolor){
      cR = ((byte) ((((((libaroma_color_r(startColor) * r_intensity)>>8)+ ((libaroma_color_r(endColor) * intensity) >> 8)))<(0xff))?((((libaroma_color_r(startColor) * r_intensity)>>8)+ ((libaroma_color_r(endColor) * intensity) >> 8))):(0xff))
                                                                );
      cG = ((byte) ((((((libaroma_color_g(startColor) * r_intensity)>>8)+ ((libaroma_color_g(endColor) * intensity) >> 8)))<(0xff))?((((libaroma_color_g(startColor) * r_intensity)>>8)+ ((libaroma_color_g(endColor) * intensity) >> 8))):(0xff))
                                                                );
      cB = ((byte) ((((((libaroma_color_b(startColor) * r_intensity)>>8)+ ((libaroma_color_b(endColor) * intensity) >> 8)))<(0xff))?((((libaroma_color_b(startColor) * r_intensity)>>8)+ ((libaroma_color_b(endColor) * intensity) >> 8))):(0xff))
                                                                );
    }
    ...
    if (useAlpha) {
      ...
      cA = ((byte) ((((((startAlpha * r_intensity) >> 8) + ((endAlpha * intensity) >> 8)))<(0xff))?((((startAlpha * r_intensity) >> 8) + ((endAlpha * intensity) >> 8))):(0xff))
