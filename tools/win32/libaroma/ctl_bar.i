...
# 27 "../../../src/aroma/controls/ctl_bar.c" 2
# 1 "../../../src/aroma/controls/../ui/ui_internal.h" 1
# 37 "../../../src/aroma/controls/../ui/ui_internal.h"
...
# 28 "../../../src/aroma/controls/ctl_bar.c" 2
# 36 "../../../src/aroma/controls/ctl_bar.c"
...
void _libaroma_ctl_bar_tools_item_free(LIBAROMA_CTL_BAR_TOOLP tool){
  if (tool){
    ...
    if (tool->icon){
      if (tool->icon_flags&0x1){
        libaroma_canvas_free_ex1(&(tool->icon), 0);
      }
      tool->icon=((void *)0);
    }
    tool->icon_flags=0;
...
byte libaroma_ctl_bar_tools_set(
  ...
  if (title){
    tools->tools[index].title = (__extension__ (__builtin_constant_p (title) && ((size_t)(const void *)((title) + 1) - (size_t)(const void *)(title) == 1) ? (((const char *) (title))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (title) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void *)0)) __retval = (char *) memcpy (__retval, title, __len); __retval; })) : __strdup (title)));
  }
  if (icon){
    if (flags&0x2){
      tools->tools[index].icon = icon;
      tools->tools[index].icon_flags = 0x2;
    }
    else{
      int icosz=libaroma_dp(24);
      tools->tools[index].icon = libaroma_canvas_new_ex(icosz,icosz,1,0,((void *)0))
      ...
      if (tools->tools[index].icon){
        ...
        tools->tools[index].icon_flags = 0x1;
      }
      if (flags&0x1){
        libaroma_canvas_free_ex1(&(icon), 0);
      }
    }
  }
  if (flags&0x4){
    tools->tools[index].icon_flags |= 0x4;
    if (flags&0x8){
      tools->tools[index].icon_flags |= 0x8;
      if (!prevchecked){
        tools->tools[index].icon_flags |= 0x10;
      }
    }
    else if (prevchecked){
      tools->tools[index].icon_flags |= 0x10;
...
byte _libaroma_ctl_bar_draw_switch(
  ...
  if (ctl->handler!=&_libaroma_ctl_bar_handler){ return 0; } _LIBAROMA_CTL_BARP me = (_LIBAROMA_CTL_BARP) ctl->internal
  ...
  if (me->tools){
    if ((i>=0)&&(i<me->tools->n)){
      ...
      if (t->icon_flags&0x4){
        byte checked=(t->icon_flags&0x8)?1:0;
        ...
        LIBAROMA_CANVASP bmask = libaroma_canvas_new_ex(h_sz,h_sz,1,0,((void *)0));
        libaroma_canvas_setcolor(bmask,0,0);
        libaroma_gradient_ex1(bmask,0,0,h_sz,h_sz,0,0,h_sz>>1,0x1111,0xff,0xff,0);
        LIBAROMA_CANVASP scv = libaroma_blur_ex(bmask,rsz,1,0);
        libaroma_canvas_free_ex1(&(bmask), 0);
        libaroma_draw_ex2(cv,scv,h_draw_x-rsz,h_draw_y,0,0,scv->w,scv->h,3,0x60,0,0);
        libaroma_canvas_free_ex1(&(scv), 0);
        ...
        if (t->icon){
          ...
          LIBAROMA_CANVASP ic = libaroma_canvas_new_ex(ico_sz,ico_sz,1,0,((void *)0));
          if (ic){
            ...
            libaroma_draw_ex2(cv,ic,h_draw_x+libaroma_dp(2),h_draw_y+libaroma_dp(2),0,0,ic->w,ic->h,1,0xff,1,iccl)


             ;
            libaroma_canvas_free_ex1(&(ic), 0);
          }
        }
        return 1;
      }
...
void _libaroma_ctl_bar_internal_draw(LIBAROMA_CONTROLP ctl){
  ...
  if (me->tdc){
    libaroma_canvas_free_ex1(&(me->tdc), 0);
    me->tdc=((void *)0);
  }

  me->tdc = libaroma_canvas_new_ex(ctl->w,ctl->h,0,0,((void *)0))
  ...
  if ((me->icon)||(me->icon_flags)){
    if (me->icon){
      ...
      if (me->icon_ismask){
        libaroma_draw_ex2(me->tdc,me->icon,libaroma_dp(16),libaroma_dp(16),0,0,me->icon->w,me->icon->h,1,0xff,1,me->itemcolor)
        ...
        libaroma_draw_ex2(me->tdc,me->icon,libaroma_dp(16),libaroma_dp(16),0,0,me->icon->w,me->icon->h,1,0xff,0,0)
  ...
  if (me->menu_id){
    ...
    if (menu_ic){
      ...
      libaroma_gradient_ex1(menu_ic,bsz,bsz,bsz,bsz,me->itemcolor,me->itemcolor,bsz,0x1111,0xff,0xff,0)
       ...
       ;
      libaroma_gradient_ex1(menu_ic,bsz,libaroma_dp(10),bsz,bsz,me->itemcolor,me->itemcolor,bsz,0x1111,0xff,0xff,0)
       ...
       ;
      libaroma_gradient_ex1(menu_ic,bsz,libaroma_dp(16),bsz,bsz,me->itemcolor,me->itemcolor,bsz,0x1111,0xff,0xff,0)
       ...
       ;
      libaroma_canvas_free_ex1(&(menu_ic), 0);
    }
  }
  if (me->tools!=((void *)0)){
  ...
  if (me->tools!=((void *)0)){
    ...
    for (i=0;i<me->tools->n;i++){
      ...
      if (me->tools->tools[i].icon_flags&0x4){
        ...
        byte is_new=0;
        if (me->tools->tools[i].icon_flags&0x2){
          ic = libaroma_canvas_new_ex(icosz,icosz,1,0,((void *)0));
          if (ic){
            is_new=1;
            memset(ic->alpha,0,ic->s);
            libaroma_draw_scale_smooth(
        ...
        if (ic){
          if(me->icon_ismask){
            libaroma_draw_ex2(me->tdc,ic,x,libaroma_dp(16),0,0,ic->w,ic->h,1,0xff,1,me->itemcolor)
            ...
            libaroma_draw_ex2(me->tdc,ic,x,libaroma_dp(16),0,0,ic->w,ic->h,1,0xff,0,0)
  ...
  LIBAROMA_TEXT textp = ((void *)0);
  LIBAROMA_TEXT textps= ((void *)0);
  ...
  if (textp){
    libaroma_text_draw_ex(me->tdc,textp,me->title_x,start_text_y,0,0,0,10,0,me->itemcolor,0,0,0)
  ...
  if (textps){
    libaroma_text_draw_ex(me->tdc,textps,me->title_x,start_text_y+textp_h,0,0,0,10,0,me->itemcolor,0,0,0)
  ...
  if ((!isactive)||(me->dc==((void *)0))){
    if (me->dc!=((void *)0)){
      libaroma_canvas_free_ex1(&(me->dc), 0);
    }
    me->dc=me->tdc;
    me->tdc=((void *)0);
  }
  me->force_draw=1;
  omp_unset_nest_lock(&(me->mutex));
}
...
byte libaroma_ctl_bar_update(LIBAROMA_CONTROLP ctl){
  if (ctl->window){
    if (ctl->window->active){
      LIBAROMA_THREAD pp;
      pthread_create(&pp,((void *)0),_libaroma_ctl_bar_req_internal_draw_thread,(void *) ctl)
                                                                    ;
      pthread_detach(pp);
    }
  }
...
void _libaroma_ctl_bar_draw(
  ...
  if (ctl->handler!=&_libaroma_ctl_bar_handler){ return ; } _LIBAROMA_CTL_BARP me = (_LIBAROMA_CTL_BARP) ctl->internal
  ...
  if (me->dc){
    ...
    if ((me->change_state<1)&&(me->change_state>0)&&(me->tdc)){
      float xstate = libaroma_cubic_bezier(0.4027,0,0.2256,1,me->change_state);
      libaroma_draw_ex2(c,me->dc,0,0,0,0,me->dc->w,me->dc->h,0,0xff,0,0);
      int h = (c->h * xstate);
      int y = (c->h>>1) - (h>>1);

      if (me->change_flags&0x2){
        ...
        libaroma_draw_ex2(c,me->tdc,0,0,0,0,me->tdc->w,me->tdc->h,0,0xff * xstate,0,0)

         ;
      }
      else{
        if (me->change_flags&0x1){
        ...
        else if (me->change_flags&0x10){
          libaroma_draw_ex2(c,me->tdc,me->title_x,(c->h>>1)-(h>>1),me->title_x,(c->h>>1)-(h>>1),me->title_w,h,0,0xff,0,0)
      ...
      if (me->change_flags&0x8){
        if (me->icon){
          ...
          if (me->icon_flags==3){
            ...
            int icon_w = libaroma_dp(me->text_gap);
            LIBAROMA_CANVASP crc = libaroma_canvas_new_ex(icon_w,c->h,0,0,((void *)0))
                           ;
            if (crc){
              ...
              libaroma_canvas_free_ex1(&(crc), 0);
            }
          }
        }
      }
    ...
    if (me->touched_state){
      if (me->touched_switch){
        if ((((&me->ripple)->touch_state[(&me->ripple)->p])>0)&&
            (((&me->ripple)->release_state[(&me->ripple)->p])<1)) {
          int switch_id = me->touched_state-10;
          if (switch_id>=0){
            if (me->tools){
              if (switch_id<me->tools->n){
                float outstate = 1.0-libaroma_cubic_bezier(0.4027,0,0.2256,1,((((((0)>(( ((&me->ripple)->release_state[(&me->ripple)->p])-0.5) *2))?(0):(( ((&me->ripple)->release_state[(&me->ripple)->p])-0.5) *2)))<(1))?((((0)>(( ((&me->ripple)->release_state[(&me->ripple)->p])-0.5) *2))?(0):(( ((&me->ripple)->release_state[(&me->ripple)->p])-0.5) *2))):(1)))



                 ;
                if (me->touched_switch>=2){
                  float xstate = libaroma_cubic_bezier(0.4027,0,0.2256,1,(((((&me->ripple)->release_state[(&me->ripple)->p])*2)<(1))?(((&me->ripple)->release_state[(&me->ripple)->p])*2):(1)))
        ...
        while(libaroma_ripple_loop(&me->ripple,&ripple_i,&ripple_p)){
          ...
          if (libaroma_ripple_calculation(
            ...
            LIBAROMA_CANVASP rdc = ((void *)0);
            if (copy_w>0){
              rdc = libaroma_canvas_new_ex(copy_w,c->h,0,0,((void *)0));
              if (rdc){
                libaroma_draw_ex2(rdc,c,0,0,copy_l,0,rdc->w,rdc->h,0,0xff,0,0);
                libaroma_draw_circle(
            ...
            if (rdc){
              ...
              libaroma_canvas_free_ex1(&(rdc), 0);
            }
          }
        }
      }
...
byte _libaroma_ctl_bar_thread(LIBAROMA_CONTROLP ctl) {
  ...
  if (me->change_start>0){
    float nowstate=libaroma_duration_state(me->change_start,200);
    if (nowstate>=1){
      is_draw = 1;
      me->change_state=1;
    }
    ...
    if (res&0x4){
      ...
      if (me->touched_state<10){
        libaroma_window_post_command(
          (((0x2&0xff) << 24) | ((me->touched_state&0xff) << 16) | (ctl->id&0xffff))
        );
      }
      else{
        ...
        libaroma_window_post_command_ex(
          (((0x2&0xff) << 24) | ((me->touched_state&0xff) << 16) | (ctl->id&0xffff)),
          sstate, me->tools->tools[tools_id].id, tools_id, ((void *)0)
        );
      }
    }
    if (res&0x8){
...
void _libaroma_ctl_bar_destroy(
  ...
  if (me->subtitle!=((void *)0)){
    free(me->subtitle);
  }
  omp_unset_nest_lock(&(me->mutex));
  omp_destroy_nest_lock(&(me->mutex));
  free(me);
}
    ...
    case 0x06:
      {
        int x = msg->x;
        int y = msg->y;
        libaroma_window_calculate_pos(((void *)0),ctl,&x,&y);
        if (msg->state==0x01){
          me->touched_state = 0;
          me->touch_bound_x = 0;
          me->touch_bound_w = 0;
          ...
          if ((res&0x2)&&
            ...
            if (me->touched_state<10){
              libaroma_window_post_command(
                (((0x1&0xff) << 24) | ((me->touched_state&0xff) << 16) | (ctl->id&0xffff))
              );
            }
            else{
              int tools_id = me->touched_state-10;
...
LIBAROMA_CONTROLP libaroma_ctl_bar(
  ...
  omp_init_nest_lock(&(me->mutex));
  if (title){
    me->title = (__extension__ (__builtin_constant_p (title) && ((size_t)(const void *)((title) + 1) - (size_t)(const void *)(title) == 1) ? (((const char *) (title))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (title) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void *)0)) __retval = (char *) memcpy (__retval, title, __len); __retval; })) : __strdup (title)));
  }
  me->text_gap = 60;
  me->bgcolor = bgcolor;
...
byte libaroma_ctl_bar_set_subtitle(LIBAROMA_CONTROLP ctl,
  ...
  if (subtitle){
    me->subtitle=(__extension__ (__builtin_constant_p (subtitle) && ((size_t)(const void *)((subtitle) + 1) - (size_t)(const void *)(subtitle) == 1) ? (((const char *) (subtitle))[0] == '\0' ? (char *) calloc ((size_t) 1, (size_t) 1) : ({ size_t __len = strlen (subtitle) + 1; char *__retval = (char *) malloc (__len); if (__retval != ((void *)0)) __retval = (char *) memcpy (__retval, subtitle, __len); __retval; })) : __strdup (subtitle)));
  }
  me->change_flags |= 0x1;
  omp_unset_nest_lock(&(me->mutex));
  if (update_now){
...
byte libaroma_ctl_bar_set_icon(LIBAROMA_CONTROLP ctl,
  ...
  byte fold_icon=1;
  if (me->icon){
    libaroma_canvas_free_ex1(&(me->icon), 0);
    fold_icon=0;
  }
  if (me->icon_flags){
    fold_icon=0;
  ...
  if (icon){
    int icosz=libaroma_dp(24);
    me->icon = libaroma_canvas_new_ex(icosz,icosz,1,0,((void *)0))
