...
# 27 "../../../src/aroma/graph/draw/path.c" 2
...
byte libaroma_path_add(LIBAROMA_PATHP path, float x, float y){
  ...
  path->p[path->n].x=x;
  path->p[path->n].y=y;
  path->max.x=(((path->max.x)>(x))?(path->max.x):(x));
  path->max.y=(((path->max.y)>(y))?(path->max.y):(y));
  path->min.x=(((path->min.x)<(x))?(path->min.x):(x));
  path->min.y=(((path->min.y)<(y))?(path->min.y):(y));
  path->n++;
  return 1;
}
...
byte libaroma_path_draw(
  ...
  if (path->n>1){
    int miny = (((0)>(floor(path->min.y)))?(0):(floor(path->min.y)));
    int maxy = (((dest->h-1)<(ceil(path->max.y)))?(dest->h-1):(ceil(path->max.y)));
    int minx = (((0)>(floor(path->min.x)))?(0):(floor(path->min.x)));
    int dwidth = (((dest->w)<(ceil(path->max.x)))?(dest->w):(ceil(path->max.x)))-minx;
    if (dwidth<1){
      return 1;
    }
...
# 263 "../../../src/aroma/graph/draw/path.c"
        ...
        if (n>1){
          ...
          for (i=0;i<n;i+=2){
            ...
            if (aliasing==1){
              ...
              if (is_mask!=2){
                line[linex]=
                  (((255)<(line[linex]+(1.0-fmod(nodes[i],1))*alphaaa))?(255):(line[linex]+(1.0-fmod(nodes[i],1))*alphaaa));
                line[linerx]=
                  (((255)<(line[linerx]+fmod(nodes[i+1],1)*alphaaa))?(255):(line[linerx]+fmod(nodes[i+1],1)*alphaaa));
              }
              else{
                line[linex]=
                  (((0)>(((int) line[linex])-(1.0-fmod(nodes[i],1))*alphaaa))?(0):(((int) line[linex])-(1.0-fmod(nodes[i],1))*alphaaa));
                line[linerx]=
                  (((0)>(((int) line[linerx])-fmod(nodes[i+1],1)*alphaaa))?(0):(((int) line[linerx])-fmod(nodes[i+1],1)*alphaaa));
              }
              ...
              if (is_mask!=2){
                for (px=linew-left;px<linew;px++){
                  cline[px]=(((alpha)<(cline[px]+alphaaa))?(alpha):(cline[px]+alphaaa));
                }
              }
              else{
                for (px=linew-left;px<linew;px++){
                  cline[px]=(((0)>(((int) cline[px])-alphaaa))?(0):(((int) cline[px])-alphaaa));
                }
              }
            }
          }
