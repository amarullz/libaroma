...
extern int gzvprintf (gzFile file, const char *format, va_list va)
                                                              ...
                                                              ;
# 27 "../../../src/aroma/utils/strings.c" 2
...
# 28 "../../../src/aroma/utils/strings.c" 2
...
static LIBAROMA_SARRAYP _libaroma_language=((void *)0);
...
char * libaroma_stristr(const char *ch1, const char *ch2, int sz){
  ...
  for (i=0;i<sz;i++){
    if ((__extension__ ({ int __res; if (sizeof (ch1[i]) > 1) { if (__builtin_constant_p (ch1[i])) { int __c = (ch1[i]); __res = __c < -128 || __c > 255 ? __c : (*__ctype_tolower_loc ())[__c]; } else __res = tolower (ch1[i]); } else __res = (*__ctype_tolower_loc ())[(int) (ch1[i])]; __res; }))==(__extension__ ({ int __res; if (sizeof (ch2[j]) > 1) { if (__builtin_constant_p (ch2[j])) { int __c = (ch2[j]); __res = __c < -128 || __c > 255 ? __c : (*__ctype_tolower_loc ())[__c]; } else __res = tolower (ch2[j]); } else __res = (*__ctype_tolower_loc ())[(int) (ch2[j])]; __res; }))){
      if (!found){
        found=ch1+i;
      }
      if (++j==strlen(ch2)){
...
byte libaroma_lang_init() {
  if (_libaroma_language != ((void *)0)) {
    ;
    return 0;
  }
  _libaroma_language = libaroma_sarray(((void *)0));
  ;
  return 1;
}
