...
# 27 "../../../src/aroma/ui/window_layer.c" 2
...
# 28 "../../../src/aroma/ui/window_layer.c" 2
...
byte _libaroma_window_layer_updatedc(LIBAROMA_WINDOWP win){
  ...
  if (!me->on_direct_canvas){
    if (me->tdc){
        ...
        if (me->sidebar_showed){
          if (me->sidebar){
            if (me->sidebar_xpos>0){
              libaroma_draw_ex2(win->dc,me->sidebar->dc,0,me->sidebar->y,me->sidebar->w-me->sidebar_xpos,0,me->sidebar_xpos,me->sidebar->h,0,0xff,0,0)
               ...
               ;
              libaroma_draw_ex2(win->dc,me->tdc,me->sidebar_xpos,0,me->sidebar_xpos,0,win->dc->w-me->sidebar_xpos,win->dc->h,0x4,245-(150*me->sidebar_xpos/me->sidebar->w),0,0)
      ...
      if (!sidebar_draw){
        libaroma_draw_ex2(win->dc,me->tdc,0,0,0,0,me->tdc->w,me->tdc->h,0,0xff,0,0);
      }
    }
  }
  omp_unset_nest_lock(&(me->mutex));
...
byte libaroma_window_layer_direct_canvas(LIBAROMA_WINDOWP win, byte state){
  ...
  if (state){
    ...
    if (me->on_direct_canvas){
      if (win->dc){
        if (!me->tdc){
          me->tdc = libaroma_canvas_new_ex(win->dc->w,win->dc->h,0,0,((void *)0))

           ;
        }
        if (me->tdc) {
          libaroma_draw_ex2(me->tdc,win->dc,0,0,0,0,win->dc->w,win->dc->h,0,0xff,0,0);
        }
      }
        ...
        case 0x06:
          {
            ...
            if (msg->state==0x01){
              ...
              if (msg->state==0x02){
                ...
                me->sidebar_req_x=(((libaroma_dp(16))>(reqx))?(libaroma_dp(16)):(reqx))
                                                                     ;
                ...
                if (me->sidebar_velocity){
                  ...
                  me->sidebar_velocity = ((((((me->sidebar_velocity)<(0.45*diff))?(me->sidebar_velocity):(0.45*diff)))>(0.05*diff))?((((me->sidebar_velocity)<(0.45*diff))?(me->sidebar_velocity):(0.45*diff))):(0.05*diff))
                                                               ;
                }
    ...
    if ((me->sidebar)&&(me->sidebar_req_x!=-1)){
      ...
      if (!me->sidebar->active){
        ...
        if (me->sidebar_velocity!=0){
          ...
          int minw=(((1)>(0.05*me->sidebar->w))?(1):(0.05*me->sidebar->w));
...
# 653 "../../../src/aroma/ui/window_layer.c"
...
# 731 "../../../src/aroma/ui/window_layer.c"
byte _libaroma_window_sidebar_message_hooker(
    LIBAROMA_WINDOWP win, LIBAROMA_MSGP msg, dwordp retval
){
  _LIBAROMA_WINDOW_LAYERP me = _libaroma_window_layer_check(win->parent);
    ...
    case 0x06:
      {
        ...
        if (msg->state==0x01){
          ...
          int x = msg->x;
          int y = msg->y;
          libaroma_window_calculate_pos(win->parent,((void *)0),&x,&y);
          if (x<win->w){
            int i;
            for (i=0;i<win->childn;i++){
    ...
    case (0x80|0x5):
      {
        ...
        if (!win->dc){
          ...
          win->bg = libaroma_canvas_new_ex(win->w,win->h,0,0,((void *)0));
...
# 988 "../../../src/aroma/ui/window_layer.c"
#pragma omp parallel for
# 988 "../../../src/aroma/ui/window_layer.c"
...
byte libaroma_window_sidebar_onslide(
  ...
  omp_set_nest_lock(&(me->mutex));
  me->slide_cb = cb;
  fprintf(libaroma_debug_output(), "I/%s: ",libaroma_debug_tag()); fprintf(libaroma_debug_output(), "Init sidebar slide callback"); fprintf(libaroma_debug_output(), "\n");;
  omp_unset_nest_lock(&(me->mutex));
  return 1;
}
