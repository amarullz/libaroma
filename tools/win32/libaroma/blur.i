...
# 27 "../../../src/aroma/graph/draw/blur.c" 2
...
float * _libaroma_blur_kernel(const int inRadius) {
  ...
  float sqrtTwoPiTimesRadiusRecip = 1.0 / (sqrt(2.0 * 3.14159265359) * inRadius);
  int r = -inRadius;
  float sum = 0.0f;
  int i;
...
byte libaroma_draw_shadow(
  ...
  LIBAROMA_CANVASP cv = libaroma_canvas_new_ex(radiusx*2+1,radiusy*2+1,1,0,((void *)0));
  LIBAROMA_CANVASP horiz = libaroma_canvas_new_ex(w,radiusy*2,1,0,((void *)0));
  LIBAROMA_CANVASP vert = libaroma_canvas_new_ex(radiusx*2+1,h,1,0,((void *)0));
...
# 75 "../../../src/aroma/graph/draw/blur.c"
...
# 75 "../../../src/aroma/graph/draw/blur.c"
  ...
  for (y=0;y<=radiusy;y++){
    ...
    int x;
    for (x=0;x<=radiusx;x++){
      byte av = (((alphamax)<((kernelx[x]*kernely[y]) * alphamax))?(alphamax):((kernelx[x]*kernely[y]) * alphamax));
      if (x!=radiusx){
        cv->alpha[ypos+cv->w-x-1]=cv->alpha[ypos+x]=av;
      }
      else{
        cv->alpha[ypos+x]=av;
      }
...
# 107 "../../../src/aroma/graph/draw/blur.c"
#pragma omp parallel for
# 107 "../../../src/aroma/graph/draw/blur.c"
  ...
  libaroma_draw_ex2(dst,cv,dx-radiusx,dy-radiusy,0,0,radiusx,radiusy,1,0xff,0,0);
  libaroma_draw_ex2(dst,cv,dx+w,dy-radiusy,radiusx+1,0,radiusx,radiusy,1,0xff,0,0);
  libaroma_draw_ex2(dst,cv,dx-radiusx,dy+h,0,radiusy+1,radiusx,radiusy,1,0xff,0,0);
  libaroma_draw_ex2(dst,cv,dx+w,dy+h,radiusx+1,radiusy+1,radiusx,radiusy,1,0xff,0,0);

  libaroma_draw_ex2(dst,horiz,dx,dy-radiusy,0,0,horiz->w,radiusy,1,0xff,0,0);
  libaroma_draw_ex2(dst,horiz,dx,dy+h,0,radiusy,horiz->w,radiusy,1,0xff,0,0);

  libaroma_draw_ex2(dst,vert,dx-radiusx,dy,0,0,radiusx,vert->h,1,0xff,0,0);
  libaroma_draw_ex2(dst,vert,dx+w,dy,radiusx+1,0,radiusx,vert->h,1,0xff,0,0);


  libaroma_canvas_free_ex1(&(cv), 0);
  libaroma_canvas_free_ex1(&(horiz), 0);
  libaroma_canvas_free_ex1(&(vert), 0);
...
LIBAROMA_CANVASP libaroma_blur_ex(
  ...
  LIBAROMA_CANVASP t1 = libaroma_canvas_new_ex(nwidth,nheight,usealpha,0,((void *)0));
  if (!t1){
    return ((void *)0);
  }
  LIBAROMA_CANVASP t2 = libaroma_canvas_new_ex(nwidth,nheight,usealpha,0,((void *)0));
  if (!t2){
    libaroma_canvas_free_ex1(&(t1), 0);
    return ((void *)0);
  }
  ...
  for (y = 0; y < height; y++) {
    ...
    for (x = 0; x < nwidth; x++) {
      ...
      int dpos = drw + x;
      if (!isMask) {
        r = ((((((r)<(0xff))?(r):(0xff)))>(0))?((((r)<(0xff))?(r):(0xff))):(0));
        g = ((((((g)<(0xff))?(g):(0xff)))>(0))?((((g)<(0xff))?(g):(0xff))):(0));
        b = ((((((b)<(0xff))?(b):(0xff)))>(0))?((((b)<(0xff))?(b):(0xff))):(0));
        t1->data[dpos] = libaroma_dither_rgb(x, y, r, g, b);
        if (usealpha) {
          a = ((((((a)<(0xff))?(a):(0xff)))>(0))?((((a)<(0xff))?(a):(0xff))):(0));
          t1->alpha[dpos] = a;
        }
      }
      else {
