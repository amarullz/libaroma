...
# 27 "../../../src/aroma/controls/ctl_progress.c" 2
...
# 28 "../../../src/aroma/controls/ctl_progress.c" 2
...
# 51 "../../../src/aroma/controls/ctl_progress.c"
  ...
  if ((me->type&3)!=0x0){
    if (me->type&0x4){
      ...
      float distance = (float)
        cos((me->growtime/600.0+1)*3.14159265359)/2.0+0.5;
      if (me->currx) {
      ...
      me->state=libaroma_duration_state(me->tick,500*4)
    ...
    me->state =libaroma_duration_state(me->tick,500)

     ;
    float bzs = libaroma_cubic_bezier(0.4027,0,0.2256,1,me->state);
...
void _libaroma_ctl_progress_destroy(
  ...
  if (ctl->handler!=&_libaroma_ctl_progress_handler){ return ; } _LIBAROMA_CTL_PROGRESSP me = (_LIBAROMA_CTL_PROGRESSP) ctl->internal
...
void _libaroma_ctl_progress_draw(
  ...
  if (me->type&0x4){
    int sz = (((c->w>>1)<(c->h>>1))?(c->w>>1):(c->h>>1));
    ...
    libaroma_draw_rect(c,
      ix, iy, iw, ih,
      libaroma_colorget(ctl,((void *)0))->control_bg,
      0xcc
    );
    ...
    if ((me->type&3)==0x0){
      ...
      int dw = iw *2;
      float easein = libaroma_cubic_bezier(0.42,0,1,1,me->state);
      float easeout = libaroma_cubic_bezier(0,0,0.58,1,me->state);
      int left, right;
...
dword _libaroma_ctl_progress_msg(
  ...
  if (ctl->handler!=&_libaroma_ctl_progress_handler){ return 0; } _LIBAROMA_CTL_PROGRESSP me = (_LIBAROMA_CTL_PROGRESSP) ctl->internal
