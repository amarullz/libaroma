...
# 27 "../../../src/aroma/controls/ctl_image.c" 2
...
# 28 "../../../src/aroma/controls/ctl_image.c" 2
...
byte _libaroma_ctl_image_thread(LIBAROMA_CONTROLP ctl) {
  if (ctl->handler!=&_libaroma_ctl_image_handler){ return 0; } _LIBAROMA_CTL_IMAGEP me = (_LIBAROMA_CTL_IMAGEP) ctl->internal
...
void _libaroma_ctl_image_destroy(LIBAROMA_CONTROLP ctl){
  if (ctl->handler!=&_libaroma_ctl_image_handler){ return ; } _LIBAROMA_CTL_IMAGEP me = (_LIBAROMA_CTL_IMAGEP) ctl->internal

  ;
 omp_set_nest_lock(&(me->mutex));
  if (me->img){
    libaroma_canvas_free_ex1(&(me->img), 0);
  }
  omp_unset_nest_lock(&(me->mutex));
...
byte libaroma_ctl_image_set_image(
 ...
 me->img=libaroma_image_ex(libaroma_stream_file(src),1,1);
 if (update){
  me->update=1;
 }
 omp_unset_nest_lock(&(me->mutex));
...
LIBAROMA_CONTROLP libaroma_ctl_image(
  ...
  LIBAROMA_CANVASP cv=((void *)0);
  if (src){
    cv=libaroma_image_ex(libaroma_stream_file(src),1,1);
  }
