...
# 27 "../../../src/aroma/controls/ctl_list.c" 2
...
# 28 "../../../src/aroma/controls/ctl_list.c" 2
...
void _libaroma_ctl_list_draw_item_fresh(
  ...
  LIBAROMA_CANVASP tcanvas=((void *)0);
  ...
  if (item->handler->draw!=((void *)0)){
    item->handler->draw(ctl,item,ccv,bgcolor,
      flag);
  }
  if (tcanvas){
    libaroma_canvas_free_ex1(&(tcanvas), 0);
  }
}
...
void _libaroma_ctl_list_free_state(LIBAROMA_CTL_LIST_ITEMP item){
  if (item->state!=((void *)0)){
    if (item->state->cache_rest){
      libaroma_canvas_free_ex1(&(item->state->cache_rest), 0);
    }
    if (item->state->cache_push){
      libaroma_canvas_free_ex1(&(item->state->cache_push), 0);
    }
    if (item->state->cache_client){
      libaroma_canvas_free_ex1(&(item->state->cache_client), 0);
    }
    free(item->state);
    ;
  }
...
byte _libaroma_ctl_list_init_state_cache(
  ...
  if (item->state){
    if (!item->state->cache_rest){
      item->state->cache_rest=libaroma_canvas_new_ex(ctl->w,item->h,0,0,((void *)0));
    }
    if (!item->state->cache_push){
      item->state->cache_push=libaroma_canvas_new_ex(ctl->w,item->h,0,0,((void *)0));
    }
...
void _libaroma_ctl_list_draw_item(
  ...
  if (item->state){
    if (item->state->normal_handler){
      libaroma_draw_ex2(canvas,item->state->cache_rest,0,0,0,0,item->state->cache_rest->w,item->state->cache_rest->h,0,0xff,0,0);
      int ripple_i = 0;
      int ripple_p = 0;
      while(libaroma_ripple_loop(&item->state->ripple,&ripple_i,&ripple_p)){
        ...
        if (libaroma_ripple_calculation(
          ...
          libaroma_draw_ex2(canvas,item->state->cache_push,0,0,0,0,item->state->cache_push->w,item->state->cache_push->h,2,(byte) push_opacity,0,0)
  ...
  _libaroma_ctl_list_draw_item_fresh(
    ctl, item,canvas,bgcolor,mi->hpad, 0x0
  );
}
...
byte _libaroma_ctl_list_dodraw_item(
  ...
  if (libaroma_ctl_scroll_is_visible(ctl,item->y,item->h)){
    word bgcolor = libaroma_ctl_scroll_get_bg_color(ctl);
    LIBAROMA_CANVASP canvas=libaroma_canvas_new_ex(ctl->w,item->h,0,0,((void *)0));
    if (canvas!=((void *)0)){
      ...
      libaroma_canvas_free_ex1(&(canvas), 0);
    }
  }
  return res;
}
...
# 440 "../../../src/aroma/controls/ctl_list.c"
...
void _libaroma_ctl_list_draw(
  ...
  while(item){
    ...
    LIBAROMA_CANVASP canvas=((void *)0);
    byte is_area=0;
    if ((item->y>=y)&&(item->y+item->h<y+cv->h)){
      ...
      canvas = libaroma_canvas_new_ex(w,item->h,0,0,((void *)0));
    }
    if (canvas!=((void *)0)){
      _libaroma_ctl_list_draw_item(
        ctl, item, canvas, bgcolor
      ...
      if (!is_area){
        libaroma_draw_ex2(cv,canvas,0,item->y-y,0,0,canvas->w,canvas->h,0,0xff,0,0);
      }
      libaroma_canvas_free_ex1(&(canvas), 0);
    }
    item=item->next;
...
void _libaroma_ctl_list_destroy(
  ...
  omp_destroy_nest_lock(&(mi->imutex));
  omp_destroy_nest_lock(&(mi->mutex));
  free(mi);
  client->internal=((void *)0);
  client->handler=((void *)0);
}
...
byte libaroma_ctl_list_del_itemp_internal(
  ...
  if (f){
    if ((f==mi->first)&&(f==mi->last)){
      mi->first=mi->last=((void *)0);
    }
    else if (f==mi->first){
      mi->first = f->next;
      mi->first->prev=((void *)0);
...
byte libaroma_ctl_list_item_position(
  ...
  int y=0;
  if (absolute){
    libaroma_window_calculate_pos_abs(((void *)0),ctl,&x,&y);
  }
  else{
    libaroma_window_calculate_pos(((void *)0),ctl,&x,&y);
  }
  int ctl_y = libaroma_ctl_scroll_get_scroll(ctl,((void *)0));
  rect->x=x;
