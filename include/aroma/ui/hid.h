...
/*
 * Typedef     : LIBAROMA_HIDCB_*
 * Descriptions: hid callbacks
 */
typedef void (*LIBAROMA_HIDCB_RELEASE) \
  (LIBAROMA_HIDP);
typedef byte (*LIBAROMA_HIDCB_GETINPUT) \
  (LIBAROMA_HIDP, LIBAROMA_HID_EVENTP);
typedef byte (*LIBAROMA_HIDCB_CONFIG) \
  (LIBAROMA_HIDP, const char *, const char *, dword);
...
struct _LIBAROMA_HID {
  voidp internal;
  /* driver callbacks */
  LIBAROMA_HIDCB_RELEASE release;
  LIBAROMA_HIDCB_GETINPUT getinput;
  LIBAROMA_HIDCB_CONFIG config;
