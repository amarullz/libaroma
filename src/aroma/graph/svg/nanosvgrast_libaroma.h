  ...
  bytep im_alpha = image->alpha + pos;
	if (cache->type == NSVG_PAINT_COLOR) {
		...
		for (i = 0; i < count; i++) {
			...
			// Blend over
			r += nsvg__div255(ia * (int) libaroma_color_r(*im_data));
			g += nsvg__div255(ia * (int) libaroma_color_g(*im_data));
				   ...
				   LIBAROMA_CANVASP dst)
{
	...
	if (dst->w > r->cscanline) {
		r->cscanline = dst->w;
		r->scanline = (unsigned char*)realloc(r->scanline, dst->w);
