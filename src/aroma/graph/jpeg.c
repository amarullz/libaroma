...
void _libaroma_jpeg_mem_source(j_decompress_ptr cinfo) {}
 ...
 */
boolean _libaroma_jpeg_mem_fill_input_buffer(j_decompress_ptr cinfo) {
 ...
 */
void _libaroma_jpeg_mem_skip_input_data(j_decompress_ptr cinfo,
                                        long num_bytes) {
  _LIBAROMA_JPEG_MEM_MGRP src = (_LIBAROMA_JPEG_MEM_MGRP)cinfo->src;
  if (num_bytes > 0) {
 ...
 */
void _libaroma_jpeg_mem_src(j_decompress_ptr cinfo, const JOCTET *buffer,
                            size_t bufsize) {
  _LIBAROMA_JPEG_MEM_MGRP src;
  if (cinfo->src == NULL) {
    cinfo->src = (struct jpeg_source_mgr *)(*cinfo->mem->alloc_small)(
        (j_common_ptr)cinfo, JPOOL_PERMANENT, sizeof(_LIBAROMA_JPEG_MEM_MGR));
  }
...
void _libaroma_jpeg_error_exit(j_common_ptr cinfo) {
  _LIBAROMA_JPEG_ERROR_MGRP myerr = (_LIBAROMA_JPEG_ERROR_MGRP)cinfo->err;
...
LIBAROMA_CANVASP libaroma_jpeg_ex(LIBAROMA_STREAMP stream, byte freeStream,
  ...
  /* Create Canvas */
  cv = libaroma_canvas_new(pcv_w, pcv_h, 0, hicolor);
  if (!cv) {
...
#ifdef LIBJPEG_HASRGB565_OUT
      jpeg_read_scanlines(&cinfo, (bytep *) &line, 1);
...
byte libaroma_jpeg_draw_ex(LIBAROMA_STREAMP stream, byte freeStream,
  ...
  if (((cinfo.output_width == vdw) && (cinfo.output_height == vdh)) ||
      (!resize)) {
    if (!resize) {
      if (vdh > cinfo.output_height) vdh = cinfo.output_height;
      if (vdw > cinfo.output_width) vdw = cinfo.output_width;
      if (vdh < cinfo.output_height) cinfo.output_height=vdh;
      if (vdw < cinfo.output_width) cinfo.output_width=vdw;
    }
#ifndef LIBJPEG_HASRGB565_OUT
...
byte libaroma_jpeg_savemem_scale(LIBAROMA_CANVASP sc, unsigned long *jpegSize,
  ...
  libaroma_draw_scale(cc, sc, 0, 0, cc->w, cc->h, 0, 0, sc->w, sc->h, 1);
