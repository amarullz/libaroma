...
int libaroma_ctl_button_width(const char *text) {
  LIBAROMA_TEXT textp =
      libaroma_text(text, 0, libaroma_fb()->w,
                    LIBAROMA_FONT(0, 4) | LIBAROMA_TEXT_SINGLELINE |
                        LIBAROMA_TEXT_FIXED_INDENT | LIBAROMA_TEXT_FIXED_COLOR |
                        LIBAROMA_TEXT_NOHR,
                    100);
  if (textp) {
...
void _libaroma_ctl_button_internal_draw(LIBAROMA_CONTROLP ctl) {
  ...
  if (me->style & LIBAROMA_CTL_BUTTON_COLORED) {
    if (is_disabled) {
      libaroma_draw_ex(btnmask, bg, 0, 0, ix, iy, btnmask->w, btnmask->h, 0,
                       0xff);
    ...
    if (!is_disabled) {
      libaroma_draw_rect(btnmask, 0, 0, btnmask->w, btnmask->h, push_color,
                         push_opa);
  ...
  dword text_alignment =
      (me->style & LIBAROMA_CTL_BUTTON_LEFT) ? 0 : LIBAROMA_TEXT_CENTER;

  /* draw text */
  LIBAROMA_TEXT textp =
      libaroma_text(me->text, text_color, iw - libaroma_dp(16),
                    LIBAROMA_FONT(0, 4) | LIBAROMA_TEXT_SINGLELINE |
                        text_alignment | LIBAROMA_TEXT_FIXED_INDENT |
                        LIBAROMA_TEXT_FIXED_COLOR | LIBAROMA_TEXT_NOHR,
                    100);
...
void _libaroma_ctl_button_req_internal_draw(LIBAROMA_CONTROLP ctl) {
  LIBAROMA_THREAD pp;
  libaroma_thread_create(&pp, _libaroma_ctl_button_req_internal_draw_thread,
                         (void *)ctl);
    ...
    if (me->push_canvas != NULL) {
      ...
      while (libaroma_ripple_loop(&me->ripple, &ripple_i, &ripple_p)) {
        ...
        if (libaroma_ripple_calculation(&me->ripple, ctl->w, ctl->h,
                                        &push_opacity, &ripple_opacity, &x, &y,
                                        &size, ripple_p)) {
          if (me->push_canvas != NULL) {
          ...
          byte push_opa =
              (byte)((((int)ripple_opacity) * (me->isdark ? 64 : 30)) >> 8);
          if (is_circle) {
            LIBAROMA_CANVASP rdc = libaroma_canvas(c->w, c->h);
            if (rdc) {
              libaroma_draw(rdc, c, 0, 0, 0);
              libaroma_draw_circle(rdc, dcolor, x, y, size, push_opa);
              int cntx = rdc->w >> 1;
              int cnty = rdc->h >> 1;
              int szxy = MIN(rdc->w, rdc->h) - libaroma_dp(12);
              libaroma_draw_mask_circle(c, rdc, cntx, cnty, cntx, cnty, szxy,
                                        0xff);
              libaroma_canvas_free(rdc);
            }
          } else {
            LIBAROMA_CANVASP ca = libaroma_canvas_area(
                c, libaroma_dp(4), libaroma_dp(6), c->w - libaroma_dp(8),
                c->h - libaroma_dp(12));
            if (ca) {
              libaroma_draw_circle(ca, dcolor, x, y, size, push_opa);
...
LIBAROMA_CONTROLP libaroma_ctl_button(LIBAROMA_WINDOWP win, word id, int x,
                                      ...
                                      byte button_style, word button_color) {
  /* init internal data */
  _LIBAROMA_CTL_BUTTONP me =
      (_LIBAROMA_CTL_BUTTONP)calloc(sizeof(_LIBAROMA_CTL_BUTTON), 1);
  if (!me) {
  ...
  LIBAROMA_CONTROLP ctl = libaroma_control_new(
      id, x, y, w, h, libaroma_dp(48), libaroma_dp(48), /* min size */
 ...
 */
byte libaroma_ctl_button_style(LIBAROMA_CONTROLP ctl, byte button_style,
                               word button_color) {
  /* internal check */
  _LIBAROMA_CTL_CHECK(_libaroma_ctl_button_handler, _LIBAROMA_CTL_BUTTONP, 0);
  libaroma_mutex_lock(me->mutex);
 ...
 */
byte libaroma_ctl_button_text(LIBAROMA_CONTROLP ctl, const char *text) {
  /* internal check */
  _LIBAROMA_CTL_CHECK(_libaroma_ctl_button_handler, _LIBAROMA_CTL_BUTTONP, 0);
  libaroma_mutex_lock(me->mutex);
  if (me->text != NULL) {
 ...
 */
byte libaroma_ctl_button_disable(LIBAROMA_CONTROLP ctl, byte is_disable) {
  /* internal check */
  _LIBAROMA_CTL_CHECK(_libaroma_ctl_button_handler, _LIBAROMA_CTL_BUTTONP, 0);
  libaroma_mutex_lock(me->mutex);
  if (is_disable) {
