...
#endif
#ifndef __libaroma_sse_neon_color_h__
#define __libaroma_sse_neon_color_h__
...
static inline void libaroma_color_copy32(
  ...
  if (n>=8){
    ...
    for (i=0;i<n-left;i+=8) {
      ...
      uint16x8_t u = vandq_u16(psrc,msk_r);
      uint8x8_t r = vshrn_n_u16(u,8);
      u=vandq_u16(psrc,msk_g);
      uint8x8_t g = vshrn_n_u16(u,3);
      u=vandq_u16(psrc,msk_b);
      u=vshlq_n_u16(u,3);
      uint8x8_t b = vmovn_u16(u);
      uint8x8_t k = vshr_n_u8(r,5);
      n_dst.val[rgb_pos[3]] = vorr_u8(r,k);
      k=vshr_n_u8(g,6);
      n_dst.val[rgb_pos[4]] = vorr_u8(g,k);
      k=vshr_n_u8(b,5);
      n_dst.val[rgb_pos[5]] = vorr_u8(b,k);
#else
      uint16x8_t u = vandq_u16(psrc,msk_r);
      n_dst.val[rgb_pos[3]] = vshrn_n_u16(u,8);
      u=vandq_u16(psrc,msk_g);
      n_dst.val[rgb_pos[4]] = vshrn_n_u16(u,3);
      u=vandq_u16(psrc,msk_b);
      u=vshlq_n_u16(u,3);
      n_dst.val[rgb_pos[5]] = vmovn_u16(u);
#endif
      /* dump it */
...
static inline void libaroma_color_copy16(
  ...
  if (n>=8) {
    for (i=0;i<n-left;i+=8) {
      ...
      uint8x8_t u = vshr_n_u8(psrc.val[rgb_pos[3]], 3);
      uint16x8_t t = vmovl_u8(u);
      uint16x8_t q = vshlq_n_u16(t, 11);
      
      u = vshr_n_u8(psrc.val[rgb_pos[4]], 2);
      t = vmovl_u8(u);
      t = vshlq_n_u16(t,5);
      q = vorrq_u16(q,t);
      
      u = vshr_n_u8(psrc.val[rgb_pos[5]], 3);
      t = vmovl_u8(u);
...
static inline void libaroma_color_copy_rgb24(
  ...
  if (n>=8){
    ...
    for (i=0;i<n-left;i+=8){
      uint16x8_t psrc = vld1q_u16(src+i);
      uint8x8x3_t pdst;
      uint16x8_t t = vandq_u16(psrc,maskr);
      pdst.val[0] = vshrn_n_u16(t,8);
      t=vandq_u16(psrc,maskg);
      pdst.val[1] = vshrn_n_u16(t,3);
      t=vandq_u16(psrc,maskb);
      t=vshlq_n_u16(t,3);
      pdst.val[2] = vmovn_u16(t);
      /* dump it */
...
static inline void libaroma_color_copy_bgr24(
  ...
  if (n>=8){
    ...
    for (i=0;i<n-left;i+=8){
      ...
      uint8x8x3_t pdst;
      uint16x8_t t = vandq_u16(psrc,maskr);
      pdst.val[2] = vshrn_n_u16(t,8);
      t = vandq_u16(psrc,maskg);
      pdst.val[1] = vshrn_n_u16(t,3);
      t = vandq_u16(psrc,maskb);
      t = vshlq_n_u16(t,3);
      pdst.val[0] = vmovn_u16(t);
      /* dump it */
      vst3_u8(dst+i*3, pdst);
    }
  }
...
#endif /* __libaroma_sse_neon_color_h__ */
