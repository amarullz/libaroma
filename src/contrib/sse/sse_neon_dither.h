...
#endif
#ifndef __libaroma_sse_neon_dither_h__
#define __libaroma_sse_neon_dither_h__
...
static inline void _libaroma_neon_dither_table16x8(
  ...
  byte table_p = ((y & 7) << 3);
  uint8x8_t n=vld1_u8(libaroma_dither_table_r()+table_p);
  *r = vmovl_u8(n);
  n=vld1_u8(libaroma_dither_table_g()+table_p);
  *g = vmovl_u8(n);
  n=vld1_u8(libaroma_dither_table_b()+table_p);
  *b = vmovl_u8(n);
}
...
static inline void libaroma_dither_line(
  ...
  if (w>=8){
    ...
    for (i=0;i<w-left;i+=8){
      ...
      uint16x8_t u = vaddl_u8(rgba.val[2], table_r);
      uint16x8_t r = vminq_u16(u,ff);
      u=vaddl_u8(rgba.val[1], table_g);
      uint16x8_t g = vminq_u16(u,ff);
      u=vaddl_u8(rgba.val[0], table_b);
      uint16x8_t b = vminq_u16(u,ff);
      r = vshrq_n_u16(r,3);
      r = vqshlq_n_u16(r,11);
      g = vshrq_n_u16(g,2);
      g = vqshlq_n_u16(g,5);
...
static inline void libaroma_dither_line_const(
  ...
  if (w>=8){
    ...
    for (i=0;i<w-left;i+=8){
      uint16x8_t u = vaddl_u8(src_r, table_r);
      uint16x8_t r = vminq_u16(u,ff);
      u=vaddl_u8(src_g, table_g);
      uint16x8_t g = vminq_u16(u,ff);
      u=vaddl_u8(src_b, table_b);
      uint16x8_t b = vminq_u16(u,ff);
      r=vshrq_n_u16(r,3);
      r = vqshlq_n_u16(r,11);
      g = vshrq_n_u16(g,2);
...
#endif /* __libaroma_sse_neon_dither_h__ */
